...
from sky.client.cli import gpus_slurm
    ...
    def _output() -> Generator[str, None, None]:
            ...
            if slurm_is_enabled and (cloud_name is None or cloud_is_slurm):
                try:
                    ...
                    slurm_realtime_infos, total_table, failed_infos = (
                        gpus_slurm.get_realtime_gpu_tables(
                            _list_to_str, slurm_cluster_name=region))
                except ValueError as e:
                    ...
                    yield from gpus_slurm.format_realtime_gpu(
                        total_table,
        ...
        if (slurm_is_enabled and (cloud_name is None or cloud_is_slurm) and
            ...
            try:
                ...
                yield from gpus_slurm.format_realtime_gpu(total_table,
                                                          slurm_realtime_infos,
                                                          failed_infos,
                                                          show_node_info=False)
            except ValueError as e:
