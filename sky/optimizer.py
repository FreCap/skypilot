...
from sky import optimizer_reporting
...
class Optimizer:
    ...
    ) -> tuple[clouds.Cloud | None, clouds.Cloud | None, float | None]:
    ...
    def _print_egress_plan(graph, plan, minimize_cost):
        return optimizer_reporting.print_egress_plan(
            graph,
            plan,
            minimize_cost,
            get_egress_info=Optimizer._get_egress_info,
            egress_cost=Optimizer._egress_cost,
            egress_time=Optimizer._egress_time,
            dummy_source_name=_DUMMY_SOURCE_NAME,
            logger=logger)
    ...
    def print_optimized_plan(
        ...
        return optimizer_reporting.print_optimized_plan(
            graph,
            topo_order,
            best_plan,
            total_time,
            ...
            node_to_cost_map,
            minimize_cost,
            print_egress_plan_fn=Optimizer._print_egress_plan,
            dummy_task_names=(_DUMMY_SOURCE_NAME, _DUMMY_SINK_NAME),
            logger=logger)

    @staticmethod
    def _print_candidates(node_to_candidate_map: _TaskToPerCloudCandidates):
        return optimizer_reporting.print_candidates(node_to_candidate_map,
                                                    logger=logger)
    ...
    def _optimize_independent(
            dag: 'dag_lib.Dag', minimize: common.OptimizeTarget,
    ...
    def _print_job_group_plan(tasks: list[task_lib.Task]) -> None:
        return optimizer_reporting.print_job_group_plan(tasks, logger=logger)

    @staticmethod
    def _find_common_infras(
        task_candidates: dict[task_lib.Task, _PerCloudCandidates]
    ) -> list[tuple[clouds.Cloud, str | None]]:
        ...
        infras_per_task: list[set[tuple[str, str | None]]] = []

        for _, cloud_candidates in task_candidates.items():
            task_infras: set[tuple[str, str | None]] = set()
        ...
        result: list[tuple[clouds.Cloud, str | None]] = []
    ...
    def _select_best_infra(
            common_infras: list[tuple[clouds.Cloud, str | None]],
            task_candidates: dict[task_lib.Task, _PerCloudCandidates],
            tasks: list[task_lib.Task],
            minimize_cost: bool) -> tuple[clouds.Cloud, str | None]:
        ...
        best_infra: tuple[clouds.Cloud, str | None] | None = None
