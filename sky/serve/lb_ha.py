...
class DemandSnapshot:
    ...
    def from_request(cls, request_data: dict[str, Any]) -> DemandSnapshot:
        ...
        return cls(
            ...
            _priority_map(request_data.get('queue_depth_by_priority')),
            _priority_map(request_data.get('rejected_in_window_by_priority')),
            _priority_map(
                request_data.get('rejected_in_recent_window_by_priority')),
            _nonnegative(request_data.get('unique_job_arrivals_60s')),
            _nonnegative(request_data.get('unique_job_arrivals_300s')),
            _nonnegative(request_data.get('headerless_arrivals_60s')),
            _nonnegative(request_data.get('headerless_arrivals_300s')),
            request_data.get('offered_arrival_tracking_saturated') is True,
        )
