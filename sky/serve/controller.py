...
class SkyServeController:
    ...
    def __init__(
        ...
        if (service_hash is not None and
                ...
                and service_spec.uses_logical_replicas is not True):
            raise RuntimeError(
    ...
    def _get_lb_replica_info(
        ...
        translation_cache = {
            ...
            if (replica_id in nonterminal_records and
                self._lb_translation_cache_record_ids.get(
    ...
    def _logical_bridge_capacity_candidates(
        ...
        if self._autoscaler.replica_unit != 'logical':
            return {}
        ...
        for info in replica_infos:
            ...
            if observed is None or observed < 1:
                continue
            current = info.planned_capacity
            durable_bound = (current
                             if info.logical_bridge_capacity_verified else 1)
            ...
            if (not info.logical_bridge_capacity_verified or
                    verified > current):
    ...
    async def _confirm_logical_bridge_capacities(
        ...
        try:
            confirmed = await loop.run_in_executor(
                None, self._replica_manager.confirm_logical_bridge_capacities,
                bridge_candidates)
        except Exception as e:  # pylint: disable=broad-except
    ...
    def _prepare_load_balancer_report(
        ...
        ha_enabled = self._lb_ha_enabled
    ...
    def _apply_prepared_load_balancer_report(
        ...
        with self._routing_state_lock:
            ...
            if (translated_in_flight is not None and
                    self._autoscaler.replica_unit == 'logical'):
    ...
    async def _handle_load_balancer_sync(
        ...
        async with lb_sync_lock:
            ...
            demand_lock = self._lb_demand_lock
        ...
        if (prefetched_state is not None and prefetched_state.enabled and
            ...
            try:
                snapshot_read = (await self._get_shared_stable_lb_role_snapshot(
                    ...
                    prefetched_database_snapshot.owner))
            finally:
                trace.add_phases({
    ...
    def _get_capacity_hint(
        ...
        logical = self._autoscaler.replica_unit == 'logical'
        ...
        for info in replica_infos:
            if info.version != latest_version or info.is_terminal:
                continue
            if logical and info.status_property.is_scale_down is True:
                continue
        ...
        min_by_accelerator = self._autoscaler.min_replicas_by_accelerator
        demand_by_accelerator = (
            self._autoscaler.target_num_replicas_by_accelerator)
    ...
    def _get_replica_counts(
        ...
        logical = autoscaler.replica_unit == 'logical'
        ...
        for info in replica_infos:
            ...
            is_zero_cost = (info.is_zero_cost is True or
                            autoscaler.is_replica_on_zero_cost_location(info))
            if status == serve_state.ReplicaStatus.READY:
                observed_ready = (autoscaler.get_ready_replica_capacity(info)
                                  if logical else width)
        ...
        demand_target = autoscaler.target_num_replicas_by_accelerator
    ...
    def _get_free_reserved_slots_by_accelerator(self) -> dict[str, int]:
        ...
        if placer is None:
            return {}
        locations = placer.zero_cost_locations()
    ...
    def _get_fill_target_by_accelerator(
        ...
        if self._autoscaler.reserved_capacity_fill is not True:
            return {}
        raw_aggregate_target = self._autoscaler.fill_target
    ...
    def _configured_accelerators(self, service_spec: Any) -> list[str]:
        ...
        floors = service_spec.min_replicas_by_accelerator
    ...
    def _build_routing_spec(
        ...
        routing_spec = {
            ...
            'target_concurrency_per_replica':
                service_spec.target_concurrency_per_replica,
            ...
            'request_queue': service_spec.lb_request_queue,
        }
            ...
            return responses.JSONResponse(content={
                'message': 'An existing dynamic_fallback_per_gpu service '
                           ...
                           'semantics. Create a new service for that migration.'
                                          ...
                                          status_code=400)
        if (validation_service.replica_unit == 'logical' and
    ...
    def _discard_retry_stage_if_unused(
        ...
        with self._update_condition:
            stage_in_use = any(update is not None and update.version == version
                               for update in (self._pending_update,
                                              self._applying_update))
        if not stage_in_use:
        ...
        with self._update_condition:
            ...
            if version > self._applied_version:
                ...
                if pending is None or version > pending.version:
                    ...
                    if pending is not self._applying_update:
                        discarded = pending
                    scheduled = True
    ...
    def _run_update_reconciler(self) -> None:
        ...
        while (not self._update_recovery_required and not stop_event.is_set()):
    ...
    def _apply_service_update(
        ...
        try:
            ...
            def _preflight() -> Any:
                if (self._autoscaler.replica_unit == 'logical' and
                        service.uses_logical_replicas is not True):
                    raise ValueError(
        ...
        try:
            ...
            reserved_capacity_fill_enabled = bool(
                service.reserved_capacity_fill)
            with self._routing_state_lock:
                if replace_autoscaler:
    ...
    def _prepare_current_paid_launch_templates(
        self,
        ...
        """Prepare the bounded identity-free paid catalog before all locks."""
        logical_max = decision_autoscaler.max_replicas
        backend_num_nodes = decision_autoscaler.backend_num_nodes
        if (type(logical_max) is not int or logical_max <= 0 or
                type(backend_num_nodes) is not int or backend_num_nodes < 1 or
        ...
        try:
            ...
            # This optimistic read resolves only immutable provider/pool
            # identities.  It cannot size the wave: the fused transaction
            # expands the catalog against its locked target and pool graph.
            paid_launch_budget = paid_capacity.build_launch_budget(
                ...
                service_name=self._service_name,
                service_hash=self._service_hash,
                requested_frontier_keys={(card,) for card in shapes},
            ...
            return ()
        try:
            prepared = self._replica_manager.prepare_paid_launch_templates(
                accelerator_shapes=shapes,
                version_authority=version_authority,
                paid_location_launch_budget=paid_launch_budget)
            if (not isinstance(prepared, tuple) or any(
                    not isinstance(template, paid_capacity.PaidLaunchTemplate)
                    for template in prepared)):
                raise ValueError(
                    'Replica manager returned malformed templates.')
            return prepared
        except (TypeError, ValueError) as error:
            logger.warning(
                'Suppressing paid launch preparation because its '
                'provider-free inputs are invalid: %s',
                common_utils.format_exception(error))
            return ()

    def _prepare_current_paid_launch_handoff_templates(
        self,
        decision_autoscaler: autoscalers.ConcurrencyAutoscaler,
        decision_version: int,
    ) -> tuple[paid_capacity.PaidLaunchTemplate, ...] | None:
        """Prepare the identity-free catalog fenced around paid admission."""
        try:
        ...
        if (max_live_paid_gpu_units is not None and
            ...
            return None
        return self._prepare_current_paid_launch_templates(
            decision_autoscaler, decision_version, max_live_paid_gpu_units)
    ...
    def _plan_and_admit_current_capacity(
        ...
        *,
        sequenced_reserved_fill: bool,
        prepared_paid_launch_templates: tuple[paid_capacity.PaidLaunchTemplate,
                                              ...],
        ...
        try:
            ...
            if (not configured_shapes.entries or
                    any(width <= 0 for _, width in configured_shapes.entries) or
                    type(backend_num_nodes) is not int or
                    backend_num_nodes < 1):
        ...
        if (not isinstance(prepared_paid_launch_templates, tuple) or
                any(not isinstance(template, paid_capacity.PaidLaunchTemplate)
                    for template in prepared_paid_launch_templates)):
            logger.warning('Suppressing promoted capacity planning because '
                           'its prepared paid catalog is malformed.')
            return None
        ...
        def _planner(
            ...
            reservation_input = self._reservation_input_from_locked_supply(
                supply)
            locked_max_live_paid_gpu_units = supply.max_live_paid_gpu_units
            prior_policy_state = supply.prior_policy_state
            prior_candidate = supply.prior_candidate
            planning_db_epoch = supply.planning_db_epoch
            if (not isinstance(prior_policy_state,
                    ...
                    not math.isfinite(planning_db_epoch) or
                    planning_db_epoch < 0):
                raise capacity_admission.CapacityAdmissionConflict(
                    'Locked durable capacity history is unavailable.')
            ...
            try:
                ...
                locked_planner_input_fingerprint = (
                    autoscalers.replica_planning_binding_fingerprint(
                        locked_decision_inputs))
                if (locked_planner_input_fingerprint
                        != supply.planner_replica_projection_sha256):
                    raise capacity_admission.CapacityAdmissionConflict(
                        'Repository and autoscaler replica projections '
                        'disagree.')
            ...
            try:
                durable_plan = (
                    decision_autoscaler.plan_durable_capacity_reconcile(
                        ...
                        retirement_shelter=retirement_shelter,
                        max_live_paid_gpu_units=(
                            locked_max_live_paid_gpu_units),
            ...
            priority_by_accelerator: dict[str, int] = {}
            for option in durable_plan.scaling_decisions:
                if (option.operator
                        is not autoscalers.AutoscalerDecisionOperator.SCALE_UP
                        or not isinstance(option.target,
                                          autoscalers.LogicalScaleTarget)):
                    continue
                for card, priority in (
                        option.target.launch_priority_by_accelerator):
                    folded = card.casefold()
                    existing = priority_by_accelerator.get(folded)
                    if existing is not None and existing != priority:
                        raise ValueError('Durable plan returned conflicting '
                                         'paid launch priorities.')
                    priority_by_accelerator[folded] = priority
            positive_paid_cards = {
                card.casefold()
                for card, count in candidate.paid_launch_target.entries
                if count > 0
            }
            if (not positive_paid_cards.issubset(priority_by_accelerator) or
                    any(
                        type(priority) is not int or
                        not serve_constants.LB_REQUEST_PRIORITY_MIN <= priority
                        <= serve_constants.LB_REQUEST_PRIORITY_MAX
                        for card, priority in priority_by_accelerator.items()
                        if card in positive_paid_cards)):
                raise ValueError('Durable plan has no exact priority for every '
                                 'positive paid accelerator target.')
            paid_launch_priorities = {
                card: priority_by_accelerator[card]
                for card in sorted(positive_paid_cards)
            }
            ...
            if (candidate.kind not in (
                    ...
                    next_policy_state is None or
                    next_policy_state.last_reduced_demand_generation
                    > demand_generation or envelope.snapshot.source_fingerprint
                    != locked_source_fingerprint):
            ...
            padding = candidate.zero_cost_padding_target.as_dict()
        ...
        with self._routing_state_lock:
            ...
            try:
                self._install_committed_capacity_plan_projection(
                    decision_autoscaler, committed.candidate)
            except (TypeError, ValueError) as error:
                logger.warning(
        ...
        return committed, planned, committed.paid_launch_specs
        ...
        *,
        ...
        if not isinstance(decision_autoscaler,
            ...
            logger.warning('Suppressing promoted capacity planning because the '
                           'autoscaler has no pure durable logical planner.')
            return None
        prepared_templates = self._prepare_current_paid_launch_handoff_templates(
        ...
        if current_plan is None:
            return None
        committed, planned, prepared_specs = current_plan
            ...
            try:
                ...
                if durable_demand_promoted:
                    ...
                    if (committed_capacity_plan.kind is capacity_planning.
                        ...
                        # autoscaler projection, but no manager target or
                        # replica effect may follow until a durable allocation
                        # witness unlocks a later generation.
                        return
                    ...
                    if self._paid_launch_receipt_underfills_plan(
                            committed_capacity_plan, paid_launch_receipt):
                        # A bounded atomic wave or locked pool window may leave
                        # residual work.  Committed members are already safe;
                            ...
                            reconcile_fresh_zero_paid_retirements(
                                paid_retirement.FreshZeroAuthority(
                                    ...
                                    capacity_plan_generation=(
                                        committed_capacity.authority.generation
                                    ),
                                    capacity_plan_sha256=(
                                        committed_capacity.authority.
                                        content_sha256),
                                    route_generation=(
                ...
                if durable_demand_promoted:
                    # The fused repository transaction is the sole promoted
                    # launch path: reserved work is admitted above and paid
                    # work is materialized only from its sparse receipt.  The
                    # planner's SCALE_UP decisions supplied priority before
                    # admission; replaying them through ReplicaManager would
                    # create a second launch path. Unknown-capacity
                    # predecessors remain protected and counted until probing
                    # or evidence-backed cleanup resolves them.
                    scaling_options = [
                        option for option in scaling_options if option.operator
                        is not autoscalers.AutoscalerDecisionOperator.SCALE_UP
                    ]
    ...
    def run(self, controller_socket: socket.socket | None = None) -> None:
        ...
        auth_required = not self._is_pool
        if auth_required:
