...
from sky.provision.gcp import tpu_node as tpu_node_lib
...
TPU_NODE_CREATION_FAILURE = tpu_node_lib.TPU_NODE_CREATION_FAILURE
...
def create_tpu_node(project_id: str, zone: str, tpu_node_config: dict[str, str],
                    vpc_name: str):
    """Create a TPU node through the legacy gcloud gateway."""
    return tpu_node_lib.create_tpu_node(
        project_id,
        zone,
        tpu_node_config,
        vpc_name,
        _format_and_log_message_from_errors,
    )


delete_tpu_node = tpu_node_lib.delete_tpu_node
# Preserve the historical facade identity for introspection and pickle.
delete_tpu_node.__module__ = __name__
