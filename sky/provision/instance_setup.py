...
from sky.provision.runtime_recovery import _install_skylet_watchdog
from sky.provision.runtime_recovery import _READ_RAY_PORT_CMD
from sky.provision.runtime_recovery import _READ_RAY_PORT_PY
from sky.provision.runtime_recovery import _REARM_AUTOSTOP_PY
from sky.provision.runtime_recovery import _runtime_recovery_head_script
from sky.provision.runtime_recovery import _runtime_recovery_install_cmd
from sky.provision.runtime_recovery import _RUNTIME_RECOVERY_SCRIPT_HEADER
from sky.provision.runtime_recovery import _RUNTIME_RECOVERY_SCRIPT_PATH
from sky.provision.runtime_recovery import _RUNTIME_RECOVERY_SERVICE_NAME
from sky.provision.runtime_recovery import _RUNTIME_RECOVERY_SERVICE_PATH
from sky.provision.runtime_recovery import _runtime_recovery_worker_script
from sky.provision.runtime_recovery import _should_install_reboot_recovery
from sky.provision.runtime_recovery import _should_install_skylet_watchdog
from sky.provision.runtime_recovery import _skylet_watchdog_install_cmd
from sky.provision.runtime_recovery import _skylet_watchdog_script
from sky.provision.runtime_recovery import _SKYLET_WATCHDOG_SCRIPT_PATH
from sky.provision.runtime_recovery import _SKYLET_WATCHDOG_SCRIPT_TEMPLATE
from sky.provision.runtime_recovery import _SKYLET_WATCHDOG_SERVICE_NAME
...
# These direct imports are the compatibility facade for historical private
# consumers and tests. Keeping explicit references also documents the surface
# that must remain available from this module.
_RUNTIME_RECOVERY_FACADE_EXPORTS = (
    MAYBE_REARM_AUTOSTOP_CMD,
    MAYBE_SKYLET_RESTART_CMD,
    _DISABLE_REBOOT_RECOVERY_ENV_VAR,
    _DISABLE_SKYLET_WATCHDOG_ENV_VAR,
    ...
    _RUNTIME_RECOVERY_SCRIPT_PATH,
    _RUNTIME_RECOVERY_SERVICE_NAME,
    ...
    _SKYLET_WATCHDOG_SCRIPT_PATH,
    _SKYLET_WATCHDOG_SCRIPT_TEMPLATE,
    _SKYLET_WATCHDOG_SERVICE_NAME,
    _SKYLET_WATCHDOG_TIMER_NAME,
...
def start_ray_on_head_node(cluster_name: str, custom_resource: str | None,
    ...
    if returncode:
        raise RuntimeError('Failed to start ray on the head node '
                           ...
                           f'===== stdout ===== \n{stdout}\n'
                           f'===== stderr ====={stderr}')
    # Ray is up: persist the exact start command (plus the idempotent
    # skylet restart) so an in-VM reboot brings the runtime back.
    # MAYBE_SKYLET_RESTART_CMD is a static command, so the full head
    # recovery script can be written here, before skylet even starts:
    # attempt_skylet starts skylet when it is not running.
    _install_runtime_reboot_recovery(head_runner,
                                     _runtime_recovery_head_script(cmd),
                                     cluster_info, log_path_abs)
...
def start_ray_on_worker_nodes(cluster_name: str, no_restart: bool,
    ...
    # The reboot recovery script always uses the no_restart variant of the
    # join command: it carries the "raylet already connected to this head"
    # guard, so a spurious run cannot kill a healthy ray worker. The head
    # internal IP is stable across in-place reboots, so the persisted join
    # command remains valid.
    recovery_cmd = (
        f'export SKYPILOT_RAY_HEAD_IP="{head_ip}"; '
        f'export SKYPILOT_RAY_PORT={ray_port}; ' + ray_worker_start_command(
            custom_resource, cluster_info.custom_ray_options, no_restart=True))
    worker_recovery_script = _runtime_recovery_worker_script(recovery_cmd)
    ...
    def _setup_ray_worker(runner_and_id: tuple[command_runner.CommandRunner,
        ...
        result = runner.run(
            ...
            source_bashrc=True)
        if result[0] == 0:
            # Ray joined the cluster: persist this worker's join command
            # so an in-VM reboot brings the runtime back.
            _install_runtime_reboot_recovery(runner, worker_recovery_script,
                                             cluster_info, log_path_abs)
        return result
...
def start_skylet_on_head_node(
    ...
    skylet_start_cmd = (f'{set_usage_run_id_cmd}; {set_skypilot_env_var_cmd}; '
                        f'{MAYBE_SKYLET_RESTART_CMD}')
    returncode, stdout, stderr = head_runner.run(skylet_start_cmd,
                                                 stream_logs=False,
                                                 require_outputs=True,
                                                 log_path=log_path_abs,
                                                 source_bashrc=True)
    if returncode:
        raise RuntimeError('Failed to start skylet on the head node '
                           ...
                           f'===== stdout ===== \n{stdout}\n'
                           f'===== stderr ====={stderr}')
    # Skylet is up: install the watchdog with the EXACT start command used
    # above (including the cluster-identity env exports), so a
    # watchdog-restarted skylet is indistinguishable from the provisioned
    # one — the bare MAYBE_SKYLET_RESTART_CMD would drop the heartbeat
    # identity env vars.
    _install_skylet_watchdog(head_runner, skylet_start_cmd, cluster_info,
                             log_path_abs)
