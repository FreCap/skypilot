...
# Keep the historical serialized function identity while moving the stateless
# directive projection behind this provisioner facade.
_SBATCH_PROTECTED_OPTIONS = getattr(sbatch_utils, '_SBATCH_PROTECTED_OPTIONS')
_build_custom_sbatch_directives = getattr(sbatch_utils,
                                          '_build_custom_sbatch_directives')
_compute_time_directive = getattr(sbatch_utils, '_compute_time_directive')
_build_sbatch_directives = getattr(sbatch_utils, '_build_sbatch_directives')
for _sbatch_helper in (
        _build_custom_sbatch_directives,
        _compute_time_directive,
        _build_sbatch_directives,
):
    _sbatch_helper.__module__ = __name__
