...
class ProviderCreateAmbiguousError(ExecutionPausedError):
    """Pause and replay one idempotent create with an unknown outcome.

    The provider request may have taken effect even though its response was
    lost or malformed.  Callers must retain the durable request association,
    reuse its exact provider idempotency token, and skip cleanup and failover.
    """
