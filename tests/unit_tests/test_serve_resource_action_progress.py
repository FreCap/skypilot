...
def _direct_launch_action(*, current_attempt: int = 0) -> kernel.ActionRecord:
    service_id = uuid.UUID('10000000-0000-4000-8000-000000000001')
    identity = kernel.ResourceActionIdentity(
        service_hash=str(service_id),
        service_incarnation=service_id,
        replica_id=1,
        replica_incarnation=uuid.UUID('20000000-0000-4000-8000-000000000002'),
        desired_generation=1,
        action_kind=kernel.ActionKind.LAUNCH)
    now = datetime.datetime(2026, 8, 3, tzinfo=datetime.timezone.utc)
    return kernel.ActionRecord(action_id=identity.action_id,
                               domain='serve',
                               resource_type='replica',
                               ...
                               kernel_state=kernel.KernelState.READY,
...
def test_direct_no_effect_builder_unmaterialized_round_trip() -> None:
    action = _direct_launch_action()
    cancelled_at = datetime.datetime(2026,
                                     8,
                                     4,
                                     1,
                                     2,
                                     ...
                                     4,
                                     tzinfo=datetime.timezone.utc)
    outcome = progress.build_serve_replica_action_direct_no_effect_outcome_v1(
        action,
        None,
        None,
        request_row_disposition='not_applicable',
        retained_terminal_request=None,
        cancelled_at=cancelled_at)

    assert outcome.cancellation.proof_kind is (
        progress.DirectNoEffectProofKindV1.UNMATERIALIZED)
    assert outcome.cancellation.no_io_prefix.count == 0
    assert outcome.provider_result.disposition is (
        progress.ProviderResultDispositionV1.CANCELLED)
    assert (progress.serve_replica_action_outcome_from_value_v1(
        outcome.canonical_value()).canonical_bytes == outcome.canonical_bytes)


def test_direct_outcome_codec_cross_rejects_other_bases() -> None:
    action = _direct_launch_action()
    outcome = progress.build_serve_replica_action_direct_no_effect_outcome_v1(
        action,
        None,
        ...
        request_row_disposition='not_applicable',
        retained_terminal_request=None,
        cancelled_at='2026-08-04T01:02:03.000004Z')
    value = outcome.canonical_value()

    with pytest.raises(ValueError, match='terminal-result member'):
        progress.ServeReplicaActionHandlerOutcomeV1.from_value(value)
    with pytest.raises(ValueError, match='crossed basis'):
        progress.ServeReplicaActionRequestFallbackOutcomeV1.from_value(value)
    crossed = copy.deepcopy(value)
    crossed['basis']['request_terminal_state'] = 'CANCELLED'
    with pytest.raises(ValueError, match='terminal state'):
        progress.ServeReplicaActionDirectNoEffectOutcomeV1.from_value(crossed)


def test_raw_invalid_journal_profile_inventory_is_exhaustive() -> None:
    assert tuple(
        profile.value
        for profile in progress.ProviderResourceActionRawInvalidJournalProfileV2
    ) == progress.PROVIDER_RESOURCE_ACTION_RAW_INVALID_JOURNAL_PROFILES_V2
    assert len(
        set(progress.PROVIDER_RESOURCE_ACTION_RAW_INVALID_JOURNAL_PROFILES_V2)
    ) == len(progress.PROVIDER_RESOURCE_ACTION_RAW_INVALID_JOURNAL_PROFILES_V2)
