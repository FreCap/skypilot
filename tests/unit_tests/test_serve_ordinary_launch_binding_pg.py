...
def _admit_generic_paid(
    database,
    *,
    replica_fields: dict[str, object] | None = None,
    ...
    with database.begin() as connection:
        ...
        state = _stored_replica_state(stored_fields)
        connection.execute(
            sqlalchemy.update(serve_state_schema.replicas_table).where(
                serve_state_schema.replicas_table.c.service_name == 'svc',
                serve_state_schema.replicas_table.c.replica_id == 3).values(
...
def _install_paid_plan_history(database) -> dict[str, object]:
    """Install one admitted plan and advance its mutable current head."""
    ...
    payload = {'normalized_demand': {'queue_depth': 1}}
    admitted_sha256 = capacity_admission.capacity_plan_content_sha256(payload)
    ...
    with database.begin() as connection:
        connection.execute(
            sqlalchemy.insert(plans).values(
                ...
                content_sha256=admitted_sha256,
                payload=payload,
                created_at=now))
        connection.execute(
            sqlalchemy.insert(heads).values(service_name='svc',
                                            generation=1,
                                            demand_feed_generation=1,
                                            receipt_watermark_sha256='c' * 64,
                                            refreshed_at=now,
                                            valid_until=now +
                                            datetime.timedelta(minutes=5)))
    return {
        'capacity_plan_generation': 1,
        'capacity_plan_sha256': admitted_sha256,
        'demand_feed_generation': 1,
        'demand_source_epoch': 1,
...
def _advance_paid_plan_head_to_zero(database) -> None:
    """Advance today's plan independently of a retained admitted claim."""
    plans = capacity_admission_schema.serve_capacity_plans_table
    heads = capacity_admission_schema.serve_capacity_plan_heads_table
    payload = {'normalized_demand': {'queue_depth': 0}}
        ...
        connection.execute(
            sqlalchemy.update(heads).where(
                heads.c.service_name == 'svc').values(
                    generation=2,
                    demand_feed_generation=2,
                    receipt_watermark_sha256='f' * 64,
                    refreshed_at=now,
                    valid_until=now + datetime.timedelta(minutes=5)))


def _paid_gcp_placement() -> dict[str, object]:
    return {
        'cloud': 'gcp',
        'region': 'us-central1',
        'zone': 'us-central1-a',
...
def _stage_ordinary_paid_cleanup(
    database,
    evidence: binding.ProviderEvidence,
) -> tuple[binding.BoundNonPoolLaunchContext, binding.TerminalEvidence, dict[
        str, object]]:
    """Build one post-effect graph with the historical priority corruption."""
    claim_fields = _install_paid_plan_history(database)
    placement = _paid_gcp_placement()
    # This consumer-level fixture installs an already-admitted immutable plan
    # graph directly.  Planner publication/provider-start validation has its
    # own production-path tests; keep this regression focused on the later
    # cleanup boundary.
    with mock.patch.object(
            binding.capacity_admission,
            'validate_paid_claim_in_connection',
            return_value=(datetime.datetime.now(datetime.timezone.utc) +
                          datetime.timedelta(minutes=5))):
        _, context, _ = _admit_generic_paid(database,
                                            replica_fields={
                                                'is_spot': True,
                                                'location': placement,
                                                'resources_override': placement,
                                            },
                                            claim_fields=claim_fields,
                                            priority=20)
    _advance_paid_plan_head_to_zero(database)

    quiesced_at = datetime.datetime.now(datetime.timezone.utc)
    terminal = binding.TerminalEvidence(status=binding.TerminalStatus.FAILED,
    ...
    with database.begin() as connection:
        associations = binding.ordinary_launch_associations_table
        association = connection.execute(
            sqlalchemy.select(associations).where(
                associations.c.association_id ==
                context.association_id)).mappings().one()
        provider_identity = binding.ordinary_paid_gcp_provider_identity(
            association, project_id='boltz-498512')
        resource_name = (f'{provider_identity["cluster_name_on_cloud"]}'
                         '-head-1234abcd-compute')
        present = evidence is binding.ProviderEvidence.PRESENT
        provider_payload = {
            ...
            'disk_ids': [],
            'instance_ids': [resource_name] if present else [],
        ...
        canonical_payload, provider_digest = (
            binding._ordinary_paid_provider_evidence(
                association,
                'svc-3',
                evidence,
                evidence_payload=provider_payload))

        connection.execute(
            sqlalchemy.update(associations).where(
                associations.c.association_id == context.association_id).
            values(
                owner_revision=association['owner_revision'] + 1,
                effect_phase=binding.EffectPhase.PROVIDER_IO.value,
                effect_phase_changed_at=sqlalchemy.func.clock_timestamp(),
                resolution=binding.Resolution.AMBIGUOUS.value,
                ambiguity_code='provider-result-uncertain',
                ...
                terminal_execution_generation=(terminal.execution_generation),
                execution_quiescence_required=(terminal.quiescence_required),
                execution_quiesced_generation=(terminal.quiesced_generation),
                execution_quiesced_at=terminal.quiesced_at,
    ...
    restart_info = serve_state.get_replica_info_from_id('svc', 3)
    assert restart_info is not None
    assert binding.paid_capacity.adopt_existing_claims(
        service_name='svc',
        service_hash='svc-hash',
    ...
    with database.begin() as connection:
        ...
        priority = connection.execute(
            sqlalchemy.select(claims.c.priority).where(
                ...
                claims.c.service_hash == 'svc-hash',
                claims.c.replica_id == 3)).scalar_one()
        assert priority == 20
        # Reproduce the nine rows already corrupted by the pre-fix restart
        # UPSERT.  The compatibility cleanup accepts only this changed byte.
        connection.execute(
            sqlalchemy.update(claims).where(
                ...
                claims.c.replica_id == 3).values(
                    priority=serve_constants.LB_REQUEST_PRIORITY_MIN))
    return context, terminal, canonical_payload


@pytest.mark.parametrize('provider_evidence', [
    binding.ProviderEvidence.PRESENT,
    binding.ProviderEvidence.ABSENT,
])
def test_ordinary_paid_cleanup_repairs_only_historical_priority_clobber(
        binding_database, provider_evidence) -> None:
    context, terminal, provider_payload = _stage_ordinary_paid_cleanup(
        binding_database, provider_evidence)

    # This is the production failure mode: restart adoption changed priority
    # 20 to 0 after the profile was frozen.
    with binding_database.begin() as connection:
        service = connection.execute(
            sqlalchemy.select(serve_state_schema.services_table).where(
                serve_state_schema.services_table.c.name ==
        ...
        with pytest.raises(binding.OrdinaryLaunchBindingConflict,
            ...
            binding._validate_profile_authority_in_connection(
                connection,
                service,
                replica,
                context.profile,
    ...
    with binding_database.begin() as connection:
        if provider_evidence is binding.ProviderEvidence.PRESENT:
            association, info = (
                binding.provider_presence_cleanup_authority_in_connection(
                    connection, context, terminal))
        else:
            association, info = (
                binding.provider_absence_projection_authority_in_connection(
                    connection,
                    context,
                    terminal,
                    expected_provider_evidence_payload=provider_payload))
        current_plan_generation = connection.execute(
            sqlalchemy.select(
    ...
    assert association['association_id'] == context.association_id
    assert info.replica_record_id == str(context.replica_record_id)
    assert current_plan_generation == 2
...
def test_ordinary_paid_frozen_cleanup_still_rejects_identity_mismatch(
        binding_database, mismatch) -> None:
    context, terminal, _ = _stage_ordinary_paid_cleanup(
        binding_database, binding.ProviderEvidence.PRESENT)
    checked_context = context
    with binding_database.begin() as connection:
        claims = serve_state_schema.paid_capacity_claims_table
        if mismatch == 'claim':
            connection.execute(
                sqlalchemy.delete(claims).where(claims.c.service_name == 'svc',
                                                claims.c.replica_id == 3))
        elif mismatch == 'claim_pool':
            pool_payload = json.loads(_CURRENT_PAID_POOL_KEY)
            pool_payload['zone'] = 'us-central1-b'
            other_pool = json.dumps(pool_payload,
                                    sort_keys=True,
                                    separators=(',', ':'))
            connection.execute(
                sqlalchemy.insert(
            ...
            connection.execute(
                sqlalchemy.update(claims).where(
                    claims.c.service_name == 'svc',
                    claims.c.replica_id == 3).values(pool_key=other_pool))
        elif mismatch == 'claim_service_hash':
            connection.execute(
                sqlalchemy.update(claims).where(
                    claims.c.service_name == 'svc',
                    claims.c.replica_id == 3).values(service_hash='other-hash'))
        elif mismatch == 'claimed_at':
            connection.execute(
                sqlalchemy.update(claims).where(
                    claims.c.service_name == 'svc',
                    claims.c.replica_id == 3).values(claimed_at=time.time() +
                                                     10))
        elif mismatch == 'capacity_plan_generation':
            connection.execute(
                sqlalchemy.update(claims).where(
                    claims.c.service_name == 'svc',
                    claims.c.replica_id == 3).values(
                        capacity_plan_generation=2))
        elif mismatch == 'capacity_plan_sha256':
            connection.execute(
                sqlalchemy.update(claims).where(
                    claims.c.service_name == 'svc',
                    claims.c.replica_id == 3).values(capacity_plan_sha256='0' *
                                                     64))
        elif mismatch == 'demand_feed_generation':
            connection.execute(
                sqlalchemy.update(claims).where(
                    claims.c.service_name == 'svc',
                    claims.c.replica_id == 3).values(demand_feed_generation=2))
        elif mismatch == 'demand_source_epoch':
            connection.execute(
                sqlalchemy.update(claims).where(
                    claims.c.service_name == 'svc',
                    claims.c.replica_id == 3).values(demand_source_epoch=2))
        elif mismatch == 'capacity_plan_accelerator':
            connection.execute(
                sqlalchemy.update(claims).where(
                    claims.c.service_name == 'svc',
                    claims.c.replica_id == 3).values(
                        capacity_plan_accelerator='a100'))
        elif mismatch == 'capacity_plan_units':
            connection.execute(
                sqlalchemy.update(claims).where(
                    claims.c.service_name == 'svc',
                    claims.c.replica_id == 3).values(capacity_plan_units=2))
        elif mismatch == 'priority_other':
            connection.execute(
                sqlalchemy.update(claims).where(
                    claims.c.service_name == 'svc',
                    claims.c.replica_id == 3).values(priority=1))
        elif mismatch == 'priority_out_of_range':
            connection.execute(
                sqlalchemy.update(claims).where(
                    claims.c.service_name == 'svc',
                    claims.c.replica_id == 3).values(
                        priority=serve_constants.LB_REQUEST_PRIORITY_MAX + 1))
        elif mismatch == 'placement':
            ...
            changed_state['resources_override'] = {
                **changed_state['resources_override'],
                'disk_tier': 'high',
            }
            connection.execute(
                sqlalchemy.update(replicas).where(
                    ...
                    replicas.c.replica_id == 3).values(
                        replica_state=changed_state))
        elif mismatch == 'pool_reference':
            bad_reference = 'paid-capacity:svc-hash:wrong-record:wrong-pool'
            profile = dataclasses.replace(
                context.profile,
                authorization_reference=bad_reference,
                digest=binding.canonical_non_pool_profile_digest(
                    context.profile.kind,
                    profile_version=context.profile.version,
                    authorization_kind=context.profile.authorization_kind,
                    authorization_reference=bad_reference,
                    authorization_generation=(
                        context.profile.authorization_generation),
                    authorization_digest=(
                        context.profile.authorization_digest)))
            checked_context = dataclasses.replace(context, profile=profile)
        elif mismatch == 'profile_digest':
            checked_context = dataclasses.replace(context,
                                                  profile=dataclasses.replace(
                                                      context.profile,
                                                      digest='0' * 64))
        else:
            # Model an externally corrupted pointer; the live PostgreSQL
            # trigger independently rejects this write.
        ...
        with pytest.raises(binding.OrdinaryLaunchBindingConflict):
            binding.provider_presence_cleanup_authority_in_connection(
                connection, checked_context, terminal)
