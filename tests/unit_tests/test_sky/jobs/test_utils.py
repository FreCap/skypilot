...
class TestControllerProcessAlive:
    ...
    def test_controller_process_alive_mismatched_start_time(self, monkeypatch):
        ...
        assert jobs_utils.controller_process_alive(record) is False

    def test_controller_process_alive_requires_started_at(self, monkeypatch):
        """Missing started_at is treated as dead without extra pid probes."""
        expected_pid = 2468
        cmdline_called = []
        pid_exists_called = []

        class _ProcessWithoutFallback:
            ...
            def cmdline(self):
                cmdline_called.append(True)
                return ['python', '-m', 'sky.jobs.controller']

            def is_running(self):
                return True

        monkeypatch.setattr(jobs_utils.psutil, 'Process',
                            _ProcessWithoutFallback)
        monkeypatch.setattr(jobs_utils.psutil, 'pid_exists',
                            lambda pid: pid_exists_called.append(pid))
        record = managed_job_state.ControllerPidRecord(pid=expected_pid,
                                                       started_at=None)
        assert jobs_utils.controller_process_alive(record) is False
        assert cmdline_called == []
        assert pid_exists_called == []
...
class TestStreamLogsByIdTaskFiltering:
    ...
    def _stub_stream_frontend(self, monkeypatch):
    ...
    def test_task_filter_by_int_matches_task_id(self, monkeypatch):
        ...
        task_row_read = mock.Mock(
            side_effect=lambda jid, task_id: task_info[task_id])
        monkeypatch.setattr(jobs_utils.managed_job_state,
                            'get_task_id_name_status_log', task_row_read)
        monkeypatch.setattr(
            jobs_utils.managed_job_state, 'get_num_tasks',
            mock.Mock(side_effect=AssertionError('count query used on found '
                                                 'task-id filter')))
        ...
        task_row_read.assert_called_once_with(job_id, 1)
    ...
    def test_task_filter_int_not_found(self, monkeypatch):
        ...
        count_read = mock.Mock(return_value=len(tasks))
        monkeypatch.setattr(
        ...
        snapshot_read.assert_called_once_with(job_id, 5)
        count_read.assert_called_once_with(job_id)

    def test_task_filter_int_missing_job(self, monkeypatch):
        """Test that an int task filter preserves the missing-job error."""
        job_id = 1
        self._stub_stream_frontend(monkeypatch)
        snapshot_read = mock.Mock(
            return_value=managed_job_state.JobLogStreamSnapshot(
        ...
        assert exit_code == exceptions.JobExitCode.NOT_FOUND
        assert msg == f'Job {job_id} not found.'
        snapshot_read.assert_called_once_with(job_id, 5)
        count_read.assert_called_once_with(job_id)
    ...
    def test_task_filter_int_does_not_match_task_name(self, monkeypatch):
        ...
        snapshot_read.assert_called_once_with(job_id, 99)
        count_read.assert_called_once_with(job_id)
