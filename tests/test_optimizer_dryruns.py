...
def test_partial_v100(enable_all_clouds):
    # AWS retired the p3.* (V100 16GB) instance types; the only remaining
    # V100 offering is p3dn.24xlarge with the V100-32GB accelerator.
    _test_resources_launch(sky.AWS(), accelerators='V100-32GB:8')
    _test_resources_launch(sky.AWS(), accelerators='V100-32GB:8', use_spot=True)
    _test_resources_launch(sky.AWS(), accelerators={'V100-32GB': 8})
...
def test_infer_cloud_from_instance_type(enable_all_clouds):
    ...
    _test_resources(instance_type='p3dn.24xlarge', expected_cloud=sky.AWS())
    _test_resources_launch(instance_type='p3dn.24xlarge')
