...
# Autodown runs INSIDE the node, as the pool ServiceAccount. The module created
# that ServiceAccount and bound nothing to it, so `-i N --down` 403'd forever
# while `sky status` still advertised the autostop. Unconditional: a pool that
# cannot tear its own nodes down is never what the caller wanted.
run "self_teardown_binds_the_service_account_to_teardown_verbs_only" {
  ...
  assert {
    condition = (
      length(kubernetes_role_binding_v1.pool_sa_self_teardown.subject) == 1 &&
      kubernetes_role_binding_v1.pool_sa_self_teardown.subject[0].kind == "ServiceAccount" &&
      kubernetes_role_binding_v1.pool_sa_self_teardown.subject[0].name == var.service_account_name &&
      kubernetes_role_binding_v1.pool_sa_self_teardown.subject[0].namespace == var.namespace
    )
    error_message = "The self-teardown binding must name the pool ServiceAccount in the pool namespace."
  }

  assert {
    condition     = kubernetes_role_binding_v1.pool_sa_self_teardown.role_ref[0].name == kubernetes_role_v1.pool_sa_self_teardown.metadata[0].name
    error_message = "The self-teardown binding must reference the self-teardown role."
  }

  assert {
    condition     = output.self_teardown_role_name == "${var.name}-self-teardown"
    error_message = "The module must expose the self-teardown role name unconditionally."
  }
  ...
  # The control-plane Role keeps its own, wider shape.
  assert {
    condition = length([
      for rule in kubernetes_role_v1.pool.rule : rule
    ...
    error_message = "The self-teardown role must not alter the control-plane role."
  }
}
