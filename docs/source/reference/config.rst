...
By default the generated ``aws ssm start-session`` (and the instance-id
lookup) use the ``AWS_PROFILE`` environment variable, if set. Set this to run
SSM under a specific named profile instead — useful when SkyPilot's default
credentials are an ambient role (e.g. a pod or instance identity on an API
server) and only the SSM traffic must target a different account, without
repointing every other AWS call via ``AWS_PROFILE``.
