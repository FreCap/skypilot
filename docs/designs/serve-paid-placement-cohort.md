...
Release `1.1.1512` still required an exact current ACTIVE report in Phase B.
During its first target-100 repeat, concurrent API PostgreSQL pool starvation
made that short-lived lease brittle and six exact bound requests failed before
provider I/O with the same authority error. Retained report rows cannot identify
