...
| Layer | Current state |
|---|---|
| Source base | `origin/improvements` at merge `b405314b`, release `1.1.1529`, including PR #1758's unified priority/deadline-weighted exact-card target, PR #1759's current-generation batch cancellation, and PR #1760's whole-wave drain seed. PR #1761 contains the exact restored-route acknowledgement fence described here; it is not yet deployed or production-proven. |
| Deployed control plane | SkyPilot `1.1.1529`, Helm revision 648. Two API, two controller, and three executor Pods are Ready and all use immutable image `255203429798.dkr.ecr.us-east-1.amazonaws.com/skypilot-nightly-boltz@sha256:481b530c75f839c463a42b338b67246cffd5160a57bf53081cfad46c8b046abc`. Helm storage remains disabled and the namespace has no PVC. |
...
| Active service | Lifecycle 119, incarnation `35aee1ae-258a-4952-a402-4555f2a4f64d`, is the temporary GCP-L4-Spot-only SLA qualification service at version 2 with paid residual cap 100 and no on-demand candidate. The high-priority wave proved SLA target actuation through the cap and zero on-demand, then a natural fresh-zero retirement exposed PR #1760's tail-tracker race. It must reach exact provider zero before the service is replaced by the final heterogeneous, utilization-gated definition. |
...
| Demand/publication ordering | PR #1756's PostgreSQL-linearized current-demand/current-supply transaction, PR #1757's semantic prepared-state fingerprint, PR #1758's deadline-weighted exact-card target, PR #1759's current-generation retirement cancellation, and PR #1760's frozen whole-wave drain seed are merged and deployed on `1.1.1529`; the superseded promoted publisher is removed. The SLA target and cap are load-proven. PR #1761's exact restored-route acknowledgement fence, supported service cleanup/recreation, and the final heterogeneous terminal campaign remain pending. |
