...
| Layer | Current state |
|---|---|
| Source base | `origin/improvements` at merge `d8654f88`, release `1.1.1528`, including PR #1758's unified priority/deadline-weighted exact-card target and PR #1759's current-generation batch cancellation. PR #1760 contains the paid-retirement wave drain-seed correction described here; it is not yet deployed or production-proven. |
| Deployed control plane | SkyPilot `1.1.1528`, Helm revision 647. Two API, two controller, and three executor Pods are Ready and all use immutable image `255203429798.dkr.ecr.us-east-1.amazonaws.com/skypilot-nightly-boltz@sha256:863c34d554f41352739db73ef93783fae14c29f33864b8e23dd2a5bf969fb233`. Helm storage remains disabled and the namespace has no PVC. |
...
| Demand/publication ordering | PR #1756's PostgreSQL-linearized current-demand/current-supply transaction, PR #1757's semantic prepared-state fingerprint, PR #1758's deadline-weighted exact-card target, and PR #1759's current-generation retirement cancellation are merged and deployed on `1.1.1528`; the superseded promoted publisher is removed. The SLA target and cap are load-proven, while PR #1760's teardown correction and the final heterogeneous terminal campaign remain pending. |
