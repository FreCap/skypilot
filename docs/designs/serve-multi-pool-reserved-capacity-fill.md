...
unchanged scheduler-authorized denominators. The control plane is SkyPilot
release `1.1.1494`, Helm revision 615. The latest test lifecycle has been
physically drained at the provider and scheduler layers and is retained in
`FAILED_CLEANUP` by two PHX H200 rows whose
durable launch effects are exactly `NOT_STARTED`; these are zero-cost reserved
rows, not cloud instances. The 2026-08-25 18:10 UTC provider-native AWS census
across all 17 enabled regions remains empty: zero nonterminated fleet
instances, zero open fleet or account-wide Spot requests, and zero live fleet
EBS volumes. The Kubernetes fleet Pods are also absent. A current provider-
native GCP refresh remains blocked on interactive credential renewal; the last
valid GCP and PostgreSQL paid-authority censuses were empty. Dashboard
`SHUTTING_DOWN` rows therefore describe stale controller history and do not by
themselves identify billable infrastructure.**

**The source correction for the two exact no-effect teardown rows is
implemented and PostgreSQL-qualified but not yet deployed. It consumes only
the already-durable terminal/quiescence proof while the service is irreversibly
shutting down; it neither calls a provider nor changes `NOT_QUERIED` to a
fabricated `ABSENT`. A fresh single-path service lifecycle, renewed dynamic
reserved denominator, exact 100-L4-Spot-GPU residual, 10,000-request exercise,
natural paid drain, and the immediate/+10/+30 provider-absence horizon remain
open.**
...
The unauthenticated aggregate infrastructure receipt at 20:41:07 UTC
independently reported both Kubernetes contexts healthy and zero physically
available GPUs: PHX H200 was 512 total / 2 non-preemptible-used /
510 preemptible / 0 available; East A100 was 64 / 1 / 63 / 0; and East
A100-80GB was 264 / 75 / 189 / 0. The service exactly owns the East preemptible
counts. The PHX preemptible total includes low-priority workloads across the
context, not only this service. At that 20:41 snapshot, `boltz-l4-fleet` owned
62 Kueue-admitted H200 replicas; the other 448 preemptible GPUs were occupied by
other workloads and were neither free nor attributed to this service. The public
response's `partial: true` was solely its independent managed-jobs status being
temporarily unavailable; both GPU cluster observations were `ok` with complete
integer accounting. This independently proves that no physical GPU was idle at
that snapshot. The exact authenticated pool observations separately define
which PHX capacity Kueue admitted and SkyPilot could spend.
...
The final +30-minute control read at 20:54:33--20:54:57 UTC retained the same
314/314 Ready non-Spot fleet and exact holdings/grants of 63 East A100, 189
East A100-80GB, and 62 PHX H200. Every non-fenced exact-card protocol-v2 round
again reported zero observed and spendable free capacity. The 32-way graph
remained bijective, with all 32 intents committed, all 32 API requests
`SUCCEEDED`, and all 32 replacements Ready/non-Spot. Paid claims and waiters
remained zero. Both load-balancer reporters were fresh and complete; queue,
in-flight, recent, and rejected request counts remained zero. All seven
control-plane Pods and both service load balancers remained Ready on the
`1.1.1477` digest with zero restarts. The independent public capacity snapshot
again reported both GPU clusters `ok` and zero available H200, A100, or
A100-80GB GPUs. This closes the reserved-fill operational horizon.

The dynamic denominator then increased without operator action. At 21:04:12
UTC, the observer published 16 additional PHX H200 slots as spendable under
unchanged Kueue policy. SkyPilot created all 16 intents in one allocation and
committed every one by 21:04:39, a 27.65-second commit span with no terminal or
expired intent. The durable graph was exactly 16 intents, records,
associations, API requests, and replicas; every request `SUCCEEDED`. Kueue
subsequently admitted all 16 Workloads by 21:05:23. By the 21:11:40 read, all
16 workers were Ready and the service had automatically advanced to 330/330 Ready,
zero-cost, non-Spot replicas: 63 East A100, 189 East A100-80GB, and 78 PHX H200.
Holdings equaled effective caps in every pool, every exact-card observation had
returned to zero observed/spendable free capacity, request telemetry remained
fresh and complete, and paid claims/waiters remained zero. This post-horizon
release is direct production evidence that the controller continuously follows
new scheduler-admitted capacity rather than converging only to a frozen test
denominator.
...
L4-only residual-demand exercise and prove reserved-first accounting, bounded
Spot-only overflow, and full provider/debit drain. The sender must reproduce
the real encrypted Platform envelope with unique result/completion-marker URLs;
a dummy body proves only queue pressure. The bearer must not be recovered from
a Kubernetes Secret.
...
At that historical snapshot, the sole correction was source-under-review and
not deployed: capture the exact durable on-cloud cluster name before teardown,
use an equality label selector
...
executor until manager shutdown. Raising the controller memory limit was not
the correction. PR #1703 later merged and deployed that source homogeneously;
the lifecycle-98 refill and final horizon at the top of this document close the
bounded-memory gate.
...
route-allowed leases, and the active load balancer has 280 routed,
materialized, freshly probed logical slots with zero unknown occupancy. East
contributes 13 A100 plus 141 A100-80GB and reports zero compatible physical
free slots. PHX contributes 126 H200; its sole fresh claimant has
`effective_cap = raw_grant = published_grant = holdings = 126` and `feed = 0`
across two consecutive broker rounds. All 126 PHX Pods are Running, Scheduled,
Ready, and `POLICY_ADMITTED` through the unchanged Kueue contract. At the same
instant, both the raw and spendable PHX observations reported five free
devices, but effective cap, raw/published grant, and holdings remained at 126,
so that point-in-time receipt authorized no feed or intent for them. This was
transient SkyPilot confirmation state, not a Kueue exclusion: by 06:44 a later
coherent read had converged to 290/290 Ready as ten more PHX workers became
confirmed. The 06:44 route contained 13 A100, 141
...
54, 65, 82, 106, 124, and 126, and the controller committed each persisted
delta in a batch. When an integer published baseline already exists, protocol
v2 requires a proposed rise above it to be corroborated by the next successful
broker round; initial publication without such a baseline is immediate. Under
healthy polling, confirmation normally adds one successful-round interval;
failed, skipped, or lock-contended rounds can extend wall-clock latency. This
filter rejected a one-sample 107-slot proposal that retracted to 106 instead of
creating a durable extra Pod. There is no second autoscaler damping layer on
the sequenced path. Observation-time raw free, already committed/unready
capacity, and current spendable feed are therefore distinct values; raw free
alone is not evidence of a stranded grant. If this convergence latency later
proves insufficient, the safe improvement is one coalesced, generation-fenced
early re-observation per physical pool after a rise, not speculative intent
creation, asymmetric grant damping, a shorter global poll without load
...
At that historical snapshot, the service had `min_replicas: 0` and
`max_live_paid_gpu_units: 0`. The only inherited paid alternative was Spot L4;
...
atomically rejects a new paid claim above the cap. Thus that deployed zero-cap
qualification configuration could not buy capacity even under stale callers or
concurrent launch attempts. The operator had approved one bounded qualification
envelope, but it remained disabled until protocol v10, controller-memory
safety, clean recreation, reserved occupancy, and provider observers passed.
Those source and reserved-fill gates later passed; the current service has the
cap-ten ceiling, while protected-endpoint authentication plus a real encrypted
canary remain necessary to exercise it. The reserved-before-paid residual and
successful teardown debit remain mandatory.
...
Released source includes the no-mount normalization, exact asynchronous Serve
ledger and service-incarnation fence, reserved-before-paid placement, and hard
paid GPU-unit cap. Revision 594 completed the PostgreSQL row-first exact-bind,
provider-local 1/100/1,000/10,000-request campaign, per-card execution proof,
zero-paid qualification fence, and controller-free request projection. Any
Boltz-side caller work remains restricted to
`ml_models/providers/skypilot/**`; changing Platform application, common-
...
The v10/memory rollout and renewed reserved-capacity proof later passed. The
remaining behavior gate is a genuinely uncovered paid L4 Spot residual,
followed by complete drain, provider absence, and debit release. The approved
exercise is
...
18 AWS and 18 GCP eligible regions, and at most one hour from first paid claim
...
Status: **merged, deployed, and production-qualified.** PR #1700 merged at
`9552669c0bbcbea9d2ee331569dc69fa4c7f0196` and is deployed in SkyPilot
`1.1.1470` / Helm revision 594. This is a correction inside the existing
PostgreSQL-first async dispatch path, not a second routing authority or a
provider retry.
...
Focused and real-PostgreSQL qualification covers capacity-only and unrelated-
route G-to-H acceptance with H persisted; selected wire/identity change and
removal; pruned G versus retained-G digest corruption; expired H never masking
an invalid G digest or selected URL; monotonic H with `H < G` rejected as
unavailable; same-version routing-spec corruption; controller incarnation and
producer lineage; existing/rejected/ambiguous attempt exclusion; exact HTTP-
code parsing; no provider send before bind; single-flight N-way refresh;
expired-head equal-fence renewal; pre-conflict equal-fence and same-generation/
different-digest rejection; exception, cancellation, drain/role-loss behavior;
no URL penalty; request-body identity; and `max_retries: 1` conflict-to-refresh-
to-one-send behavior. Two independent adversarial source reviews passed.

The homogeneous revision-594 production campaign then completed staged 1,
100, 1,000, and 10,000-request phases. The final phase produced exactly 10,000
terminal successes, exactly 10,000 HTTP 200 prediction responses, zero non-200
responses, zero receipt-recovery lookups, and zero failed, missing-artifact,
completion-timeout, or unsettled requests while route heads advanced. It
observed logical-device saturation and successful work on A100, A100-80GB, and
H200. No route-bind conflict, stale-route starvation, duplicate row, duplicate
provider send, provider timeout, or paid launch appeared. Genuine incompatible
movement and all fail-closed retry exclusions remain source/real-PostgreSQL
proofs; production did not inject a fake incompatible route into the live
service.

The private qualification evidence is rooted at
`/data/fsx/users/francesco/agents/evidence/boltz-l4-fleet/20260824-route-fence-rev594`.
The 10,000-request manifest SHA-256 is
`f751f71dd8f9852c5de2df578a1141e5cdcf38018e7a1e389c40a652dd837398`,
and its event journal SHA-256 is
`0bb00d1cdcd00e22437cbd027debcdc521a52d20af3232515b9df534b73accd9`.
The journal reached 179 running slots, zero minimum free slots, and queue depth
15. Those are authenticated load-balancer/harness observations, not a nonzero
`/serve/{service}/demand` or visual-dashboard sample.
...
The following chronology is retained as evidence for superseded releases and
incidents. Its old rollout instructions and point-in-time counts are not
current deployment authority.

PR #1679 merged at
`40a0832bd88f808177f8ee330b96a6d50a80f832`; immutable release `1.1.1449` was
deployed as Helm revision 573 on all two API, two controller, and three
executor writers at image
`sha256:b4d62a6cb721fa6d41c2e5cbb22d6b14bfd3540fe69427ced1a73ccf3a54ad84`
and chart digest
`sha256:ea69f7f65161d848047c2ab16ae7d0f4c3461786f7e618fc9a2418d34c1f6e8d`.
The supported typed purge retired lifecycle 93 without manual row, Pod, or
Workload deletion. `boltz-l4-fleet` was recreated from the canonical no-EFS
definition as lifecycle 94/version 1 and activated exactly once at
`SEQUENCED_ACTIVE` generation 35. The historical 34-to-35 CAS is complete and
must never be repeated.

PR #1680 merged at `b311dd2775c150895918121cbf2b16c0ba21f5dd`. It bounds
route-promotion owner-lock waiting and amortizes one physical-cluster preflight
across a 32-intent lease batch while yielding the manager mutex every four
intents. Immutable release `1.1.1450` is deployed as Helm revision 575 on the
same seven writers at image
`sha256:09424c03eb16b63216fbfa55afeded1494a654f5e8a23b3f5f8a3b1b364778da`
and chart digest
`sha256:9937b49f32c56663ff4831201ae213e33194fd8068b726f312f885414e26388c`.
...
decode/settlement/cleanup-only and has no retained renderer. PR #1683 merged at
`d476a40005c300a06fa980beef5c58337926f9aa`; immutable release `1.1.1453` is
homogeneously deployed on the same seven writers as Helm revision 577, with
image digest prefix `sha256:357dec` and chart digest prefix `sha256:804747`.
Exact pre-rollout init-plus-`subPath` canaries passed in both PHX and East
against Kubernetes API readback and were removed together with their unique
empty cache leaves.

Fresh lifecycle 95 exposed one additional cache-leaf invariant at fleet
scale.  The attested parent and every intermediate component were safe, but
one PHX node retained the exact registered, root-owned service leaf at mode
`0777` from an earlier writer.  Protocol v8 rejected that leaf forever.  Since
an init-failed Pod releases its unchanged Kueue quota and consumes no GPU, the
next exact-node placement selected the same still-free node and repeated the
failure.  This is a SkyPilot bootstrap normalization defect, not a Kueue
denial.  Protocol v9 is the steady-state correction and owns one additional
idempotent operation: after securely walking root-owned, non-symlink
components beneath the attested parent, it may tighten the final root-owned
service leaf from an otherwise ordinary directory mode to exact `0755`, then
re-read and validate it.  It must never repair an intermediate component, a
non-root-owned leaf, a non-directory, a symlink, or any set-id mode.  This
removes the permanent retry loop without weakening the parent/path trust
boundary or requiring an operator to mutate node filesystems.

Protocol v8 retains its exact released bootstrap bytes and exact adoption
validator.  Editing that script beneath the v8 discriminator would make one
durable identity name two concrete Pod renderers and would cause a new
controller to reject a healthy v8 Pod during retry/adoption (and an old
controller to reject a v9 Pod).  New projections therefore emit v9 only;
current code decodes, renders, and attests v8 exactly only to adopt an exact
already-persisted Pod identity, then settles and cleans it; fresh v8 Pod
creation remains forbidden.  An old source cohort fails closed on v9 until the
homogeneous writer rollout and new source-cohort activation.  Focused tests
must cover exact old-v8 adoption, v9
normalization and idempotence, unsafe-owner rejection, intermediate-component
rejection, exact post-normalization mode, and current-protocol rendering.

PR #1684 merged at `ca47bd868` and immutable release `1.1.1454` is deployed as
Helm revision 578 on all seven writers. Its typed whole-service purge retired
lifecycle 94 normally. `boltz-l4-fleet` was recreated from the canonical
zero-minimum/no-EFS definition as lifecycle 95/version 1, and the
generation-37-to-38 activation committed exactly once. That activation and
every earlier activation must not be repeated.

Post-activation qualification exposed three additional SkyPilot control-plane
defects alongside the cache-leaf normalization defect; none is a Kueue quota
refusal. First, a direct lifecycle-95 ledger read found more than 975
provider-free intents terminalized as `insufficient_actuation_window`. The
repository required each current observation grant to cover the full 90-second
executor ownership lease even though atomic materialization already revalidates
`valid_until > clock_timestamp()`. A current short-lived grant must be leasable;
the lease is ownership only and never extends provider authority beyond the
grant's exact expiry. Second, the sequenced demand observer still invoked the
legacy ambient provider refresh, colliding with the protocol-v2 physical fence
and producing provider-phase timeouts instead of reading the current
PostgreSQL-authoritative observation. Third, exact post-teardown provider
absence was proved and then discarded before a redundant second provider read;
a fence collision on that second read could strand an otherwise clean row.

Historical incident, now resolved: the controller deployment was temporarily
held at zero replicas to stop the cache-init failure from creating an unbounded
failed-Pod retry loop. Ready
workers, the API, both load-balancer slots, lifecycle 95, and gate 38 remained
available; PostgreSQL reported zero paid claims and zero paid waiters. The cache
correction and the three control-plane corrections were subsequently assembled
with the paid-residual atomicity correction below into a SkyPilot-only
fix-forward release. The production receipt above supersedes this hold and its
then-open deployment, reauthorization, reconciliation, and occupancy gates.

The subsequent supported recreation produced lifecycle 95 and completed the
one-way generation-38 activation. At that time its protocol-v8 launches were
being adopted, but the live controller also exposed a duplicate observation
path:
ordinary-demand budgeting and status telemetry continued to schedule the
pre-sequenced `ambient-legacy` context observer while the canonical physical
pool observer held protocol-v2 provider authority. The resulting 30-second
provider-phase timeouts and refresh-thread churn are a SkyPilot control-plane
defect, not a Kueue denial or unavailable GPU capacity. The single-observer
correction documented below was source- and focused-unit-qualified at that
point. Its then-open homogeneous deployment and live absence checks are
historical; the production receipt above is current.

Adversarial restart review then found a second source-only gap in that
correction: an exact-current service claim is sufficient for controller
restart, but cannot bootstrap a clean service or the next service version.
The same change now adds a typed, provider-free projection of the physical
context/UID/card/width inventory already embedded in and identity-hashed by the
deployment reclaim-policy bundle. Clean creation and forward version update
intersect that exact gate-bound inventory with the new version's immutable
server-owned worker projections, then require the normal `V2_FENCED` observer
and existing fresh-proof claim authorization before replacing a claim. No
ambient discovery, new database table, policy-identity rotation, or
infrastructure change is introduced. This bootstrap is source- and focused-
unit-qualified at that point; its deployment and clean-recreation gates were
later completed and are retained here only as chronology.
...
generation-38 activation preflight bound its PHX pull plus bash, Python 3, and
`findmnt` checks.

Final paid-residual review found that a previously published positive capacity
plan named the committed zero-cost counts but did not name the exact reserved-
fill allocation that made those counts current. Allocation expiry or
replacement could therefore occur after plan publication and before a paid
claim or provider effect revalidated the plan. The steady-state correction is
schema-free and PostgreSQL-atomic. The elected immutable `version_specs.spec`
is locked and decoded as the server-owned `SkyServiceSpec` discriminator, so
ordinary fill-disabled services remain unaffected even though fresh central
services use durable actuation state. Every positive fill-enabled
`DURABLE_INTENT` plan embeds the canonical full
...
1. Merge, build, and deploy the whole-service cancelled
   `PRE_EFFECT_TERMINAL` correction homogeneously to both API writers, both
   Serve controller writers, and all three request executors with direct Helm
   `--reuse-values`. Require one immutable image digest on every writer. This
   correction has no schema, migration, reconciliation-gate activation, Kueue,
   provider, platform, Terraform, or storage step.
2. Retry the supported typed purge of current `boltz-l4-fleet` lifecycle 102.
   Require replicas, admissions, and intents 49/50 to disappear in one atomic
   boundary; require the service graph absent; and retain both association and
   terminal request audit rows byte-for-byte with
   `provider_evidence=NOT_QUERIED`. Prove the purge performs zero provider,
   Pod, or physical-cluster probe and leaves zero paid claim, queue row,
   retention pin, waiter, load balancer, or provider resource. Do not manually
   reinterpret, delete, or rewrite any row or Kubernetes object.
...
This was the executable rollout sequence at the time it was written. It is now
complete and retained only as a decision receipt. Do not repeat any step,
teardown, recreation, activation CAS, or generation transition in this section.
Every later sequence that calls lifecycle 93 current, calls protocol v7
unpublished, calls Helm 572/573 current, or instructs a generation 34-to-35
transition is likewise historical and non-executable even if its local wording
still says otherwise.

1. Keep lifecycle 95 fill held. Re-read the exact service, writer, provider,
   paid, route, gate-38, and unchanged 40-object Kueue state before deployment.
   Treat generation 38 and every earlier activation as complete; never repeat
   one and never reopen legacy/direct authority.
2. Merge, publish, and direct-Helm deploy the combined cache-v9, short-grant,
   sequenced-observer, teardown-receipt, and allocation-bound paid-residual
   corrections homogeneously across all seven writers with `--reuse-values`.
   This restores the controller Deployment to its stored two-replica value.
   Mixed writers must remain fail-closed. Add no migration, EFS/PVC, Kueue,
   Terraform/Terragrunt, KubeRay, IAM, platform runtime pin, or task-placement
   change.
3. While the source-cohort receipt is still stale at generation 38, prove all
   seven live writers have the one new digest and that fresh effects remain
   held. Use the supported whole-service typed down/purge to retire lifecycle
   95 and its protocol-v8 claim and provider state with
   `sky serve down boltz-l4-fleet --purge -y`; never delete individual
   rows, Pods, Workloads, requests, or claims manually. Require lifecycle,
   service, provider, and claim absence before recreation.
4. Only after homogeneous deployment and complete typed purge, verify there is
   no service, provider object, claim, paid claim, or paid waiter for the old
   lifecycle. With that exact empty claim scope, perform exactly one
   source-cohort reauthorization from generation 38 to 39. Never repeat that
   CAS. The generation-39 policy receipt covers the complete static provider
   inventory, including contexts without a pre-existing claim.
5. At gate 39, recreate `boltz-l4-fleet` from the canonical no-EFS,
   zero-minimum definition. The fresh service must persist immutable protocol-v9
   worker projections. No second activation is required: clean service identity
   bootstraps from the already-attested static provider inventory.
6. Prove the fresh claim is bootstrapped only by the poller's gate-bound
   deployment inventory, then observed under `V2_FENCED` and committed in
   PostgreSQL before demand can consume it. Repeated demand-budget and status
   reads must use only an integrity-valid exact-current claim plus its latest
   authoritative physical-pool observations. Require zero `ambient-legacy`
   provider-phase entries, waiters, timeout logs, or refresh threads; missing
   evidence must remain UNKNOWN with zero speculative or paid allowance.
7. Prove the `insufficient_actuation_window` counter no longer grows, a current
   short grant leases and commits before expiry, and every materialization still
   fails closed after its exact `valid_until`. Prove cache leaves converge to
   exact `0755` and exact teardown absence is not followed by a second provider
   read.
8. Prove both load-balancer slots repeatedly resolve an eligible fresh route
...
9. Prove the fresh East scheduler-fit denominator and the fresh PHX
...
10. Require zero paid claims, zero paid waiters, and zero provider Spot capacity
   whenever compatible reserved supply covers demand. Require every positive
   durable capacity-plan head to carry its exact current allocation identity
   and every zero head to be an unbound revocation. Then reconnect only the
   endpoint value, send the harmless authenticated read, and repeat the census.
11. Run immediate, +10, +30, and full stale/quiescence-horizon verification and
...
| Scheduler-authorized reserved fill | Complete | Complete through `1.1.1477` / Helm 599 | Complete at gate 39 | Complete against the dynamic denominator. The exact 32-GPU wave and +30 horizon held 314/314 Ready; a later unchanged-policy release automatically advanced the service to 330/330: East 63 A100 plus 189 A100-80GB, and PHX 78 Kueue-admitted H200. All exact-card observations report zero spendable free capacity, every replica is reserved/non-Spot, and Simone's Kueue policy is unchanged. |
...
| Protocol-v8 attested cache-leaf bootstrap (#1683) | Complete at `d476a4000` | Complete in `1.1.1453` / Helm 577 | Generation 38 activation is complete and must not be repeated | Lifecycle-95 scale exposed the retained `0777` final-leaf retry defect that protocol v9 corrects; v8 remains adoption/settlement/cleanup-only. |
| Protocol-v9 cache-leaf normalization | Complete; v8 remains byte-exact only for exact persisted-Pod adoption/cleanup | Complete and inherited by `1.1.1457` / Helm 586 | Complete at gate 39 | Lifecycle 96/version 1 reached the current scheduler-authorized occupancy without the historical cache-leaf retry hold. |
| Allocation-bound paid residual | Complete; no schema or migration | Complete through PR #1686 in `1.1.1456`, inherited by `1.1.1457` | No separate activation; the immutable fill-enabled spec plus durable service mode selects the exact contract | Reserved supply and its pending/allocation tail are committed before paid residual. The current production census has zero paid claims and zero Spot replicas while compatible reserved supply covers the denominator. A genuinely uncovered paid-Spot launch/drain exercise remains open. |
...
| Whole-service cancelled pre-effect retirement | Source and real-PostgreSQL qualification complete | Pending | No activation; teardown-only | Production rows 49/50 are zero-cost `NOT_STARTED` launches with exact terminal/quiescence and terminal-request receipts. The correction must be deployed, the supported purge retried, and the retained UI rows proved absent without any provider call or `ABSENT` evidence write. |
...
complete and non-repeatable. At that historical checkpoint, closeout required
homogeneous protocol-v8 deployment, normal lifecycle-94 teardown, clean
protocol-v8 same-name recreation, one generation-37-to-38 authorization,
repeated HA promotability, fresh-capacity assignment, a zero-paid census,
endpoint reconnection, and the complete stale horizon. Those gates were later
completed and are retained here only as chronology. A deliberately billable
uncovered-demand case was not required for that historical rollout closeout.
It is now required separately to close the product-level reserved-first plus
bounded-Spot-overflow behavior; source and real-PostgreSQL tests alone do not
constitute that live proof.
...
separately reviewed 36-to-37 rotation completed. The former protocol-v8
37-to-38 closeout is also complete; the production receipt at the top of this
document is current.
...
edge remain closed for a live service.

The later supported lifecycle-94 whole-service teardown exposed the same
pre-job shape at the irreversible deletion boundary. After the provider and
request teardown had removed every East and PHX fleet Pod, 21 PHX rows remained
`FAILED_CLEANUP` with no recorded service job: 16 retained an exact
`POLICY_ADMITTED` receipt and five retained an exact expired `POD_WAITING`
receipt. Each row was off-route and unmaterialized, with one COMMITTED intent,
one terminal and executor-quiesced protocol-v2 association, canonical
post-quiescence provider `ABSENT`, and zero paid, queue, or pin authority. For
all 21, provider absence was published after the final admission receipt. The
old finalizer incorrectly treated either retained Pod receipt as proof that a
service job must have existed; the receipt proves only what Kueue had observed
before the pre-job launch stopped.

Whole-service cleanup therefore has one additional typed, validation-only
path before the normal exact-Pod loader. It accepts a retained
`POLICY_ADMITTED` or expired `POD_WAITING` pre-job row only while the exact
service lifecycle is `SHUTTING_DOWN` or `FAILED_CLEANUP`, only when the
association is still in `NOT_STARTED` or `PROVIDER_IO` with a null service job,
and only after the same complete graph, canonical Pod receipt/digest, terminal
quiescence, frozen profile, and canonical provider-absence checks pass. The
provider `ABSENT` publication must be at least as new as the admission row's
final `updated_at`. A waiting receipt additionally requires
`observed_at <= updated_at < valid_until` and PostgreSQL time at or after
`valid_until`; a still-fresh wait never authorizes retirement. The loader
returns only `ALREADY_PROVEN`, performs no provider I/O, and is revalidated by
the atomic whole-service retirement transaction. Materialized jobs remain on
the normal exact-Pod path; live-service, malformed, fresh-waiting,
non-`ABSENT`, paid, queued, pinned, foreign, or ambiguous graphs remain closed.
...
complete 180-second stale/quiescence horizon are production-proven. At that
historical point, the live gate was v7 finalization of lifecycle 93, fresh same-
name recreation, generation-35 activation, new scheduler-authorized
convergence, a zero-paid final census, and rollout-audit acceptance; all were
later completed. A billable nonzero-demand exercise was not required for that
version rollout. The current product closeout supersedes that narrower
criterion and requires one bounded live Spot residual plus full drain.
Serve057, exact replay, typed proof pause, pre-materialization proof, and the
admissionless-retirement contract are merged, deployed, and exercised.
The separate PostgreSQL idempotency/completeness contract for exact completed
logical requests described below was later implemented and provider-local
qualified on revision 594.
...
protocol-v7/cohort-7 lifecycle-93 cleanup, fresh recreation, generation-35
activation, readiness settlement, and final audit were later completed. Paid
behavior is source- and real-PostgreSQL-test-qualified; the current product
closeout separately requires one explicitly bounded genuine Spot residual and
full drain.
The steady state
...
but a lane leases only one just-in-time actuation quantum of at most four
fair-ordered actionable intents: untouched `GRANTED` rows first, then
least-recently transitioned `RETRYABLE` rows. It opens one V2 provider-phase
admission plus one
...
continues to durable projection. That successful read emits one typed receipt
binding the immutable physical-cluster fence and exact replica name. Bound
reserved-fill cleanup authenticates that receipt against the retained replica
row and passes its canonical `ABSENT` payload directly to the PostgreSQL
evidence reducer. It must not repeat the provider read between successful down
and projection: transient fence contention on a redundant read cannot weaken
already-proven exact absence back to `UNKNOWN`. `PRESENT`, `UNPROVEN`
(including provider failure or a retargeted/replaced physical cluster),
provider-phase timeout, or ownership loss before the receipt retains the
cleanup graph and capacity debit. The deadline does not reissue the already
successful down, and replay of already committed absence remains idempotent
through the existing durable projector. Legacy and non-Kubernetes cleanup are
unchanged.

Lifecycle 95 exposed the redundant-read violation under concurrent fill. The
first post-delete loop proved physical `ABSENT`, but bound cleanup immediately
performed another UID and Pod inventory. Physical-fence contention made that
second read `UNKNOWN`, retained the exact row in `SHUTTING_DOWN`, and retried
cleanup despite the preceding proof. The typed handoff removes only that
duplicate read; it does not relax ambiguous-launch quarantine, request
quiescence, lifecycle ownership, provider-generation matching, or physical-UID
fencing.
...
Whole-service teardown has one permanent retirement boundary with two mutually
exclusive proof shapes. Both apply only while the exact current lifecycle is
`SHUTTING_DOWN` or retrying `FAILED_CLEANUP`, and both re-prove the service
hash/lifecycle/owner, immutable COMMITTED intent, replica-record UUID, provider
generation, single protocol-v2 association, terminal launch generation, copied
execution-quiescence receipt, and absence of request queue rows, retention
pins, and paid claims.

If a provider effect may have begun, an association that stopped before
recording a service job additionally requires a fresh uncached physical-UID-
fenced provider read that began after quiescence and found the exact cluster
name ABSENT. The provider read occurs outside database locks. Its PostgreSQL-
clock start token and canonical physical-absence envelope are then persisted
on the association, and the complete graph is locked and revalidated before
the replica and intent are deleted atomically. `PRESENT`, `UNPROVEN`, expired
evidence, a concurrent materialization or successor, or any identity mismatch
retains the graph. Its admission may be absent, an exact field-empty
`INTENT_PENDING` row, an exact `POLICY_ADMITTED` row with canonical
namespace/name/UID receipt and digest, or an exact expired `POD_WAITING` row
with the same receipt identity. Every retained Pod receipt must predate the
canonical provider-absence publication; the waiting form must also have a
canonical 15-second lifetime, publication strictly inside that lifetime, and
PostgreSQL-confirmed expiry. These forms authorize no Pod-name inference: the
separate canonical physical-cluster `ABSENT` envelope is still mandatory. No
admission row is synthesized or backfilled.

If the durable association instead has exact `PRE_EFFECT_TERMINAL` resolution
and reconciliation, `effect_phase=NOT_STARTED`, no service job or result, an
exact terminal cause/status/generation, and quiescence through that same
generation, the effect boundary itself proves provider cleanliness. This
shape requires the unique exact terminal request receipt and one exact field-
empty `INTENT_PENDING` admission with no Pod identity or receipt. Its
cancellation is either wholly absent or one complete reason/timestamp pair;
the reason is diagnostic rather than retirement authority because the
irreversible lifecycle fence and no-effect/quiescence proof provide that
authority. Any nonempty reason of at most 128 characters is retained exactly;
partial or malformed cancellation remains closed. Provider evidence must stay
exactly `NOT_QUERIED` with no timestamp, payload, or digest. The observer
returns `ALREADY_PROVEN`, performs no Pod, cluster, or provider read, and never
manufactures `ABSENT`. The final transaction locks and revalidates the same
complete graph, deletes admission then replica then intent atomically, and
retains the association and request tombstones byte-for-byte. A live service
cannot consume a cancelled form, and any retained Pod authority, missing or
nonterminal request, queue row, pin, paid claim, provider-evidence mutation, or
identity mismatch retains the entire graph.
...
freshness is deployed. Revision 594's current-demand API projection was fresh/
complete with two reporters after the guarded campaign, while the exact
PostgreSQL ledger reconciled the provider-local cohort and remained explicitly
protocol-covered/partial. The same campaign did not capture a nonzero visual
UI sample. PR #1671 preserved the single current-demand counter path, and the
exact ledger later added terminal idempotency/completeness rather than a second
live-occupancy authority. Lifecycle 93 historically closed the nonzero healthy-
controller freshness/rendering proof; the projection also remained fresh and
complete through the PR-#1678 cross-Pod takeover and full stale interval.
Artificial provider-stall injection remains source/test-qualified and is not a
...
and dashboard/request-telemetry freshness. For an exact
`PRE_EFFECT_TERMINAL`/`NOT_STARTED` association, the provider-free contract
requires the unique terminal request, same-generation execution quiescence,
and one field-empty `INTENT_PENDING` admission, preserves exact
`NOT_QUERIED`, and proves repeated read/observer calls perform no write or
provider/Pod/cluster call. For every launch that may have crossed the provider
boundary, whole-service teardown accepts an absent or pending admission only
after a fresh, uncached, physical-UID-fenced ABSENT observation begun after
exact execution quiescence, and accepts a mixed batch of exact retained
`POLICY_ADMITTED` and expired `POD_WAITING` receipts only with an already-
canonical post-quiescence provider-absence envelope newer than each receipt
and no service job. Mixed-class batches retire atomically. Negative tests
retain the complete graph for a live service, ordinary scale-down, retained Pod
authority on the no-effect shape, missing or mismatched terminal request or
quiescence, malformed cancellation, any provider-evidence mutation on the no-
effect shape, `PRESENT`/`UNPROVEN`, expired or pre-quiescence evidence,
queue/pin/paid authority, wrong service hash/lifecycle/replica-record/provider
generation, a fresh waiting receipt, a receipt newer than provider absence, a
malformed admission receipt or frozen profile, and a concurrent admission or
...
Historical incident boundary (non-executable): lifecycle 93 reached
`FAILED_CLEANUP` with nine retained `UNKNOWN` rows after direct East and PHX
provider inventories both reported zero workloads. The v6 control plane could
not finalize those rows. At that boundary Helm revision 572 remained at
reconciliation generation 34 and the service was not active. PR #1679 later
completed the supported cleanup and fresh lifecycle-94 recreation described in
the historical closeout above.
...
passed at 03:13:53 UTC. At that historical checkpoint, overall acceptance
remained pending while the coincident eight-card refill settled and the rollout
audit verified the final paid census; later receipts closed it.
...
clean service was recreated as lifecycle 93/version 1; its later supported-down
`FAILED_CLEANUP` boundary is historical. The live service is lifecycle 98,
controller incarnation `ce80a485-8190-436b-bda6-a665aaf57fbe`, version 1. PR
#1670 and fix-forward PR
...
Historical v7 purge/recreation sequence (completed and non-executable):
...
after that cleanup's own zero-legacy/null-writer predicates pass. At this
historical checkpoint, exact completed HTTP exchanges or model jobs still
required the separate PostgreSQL idempotency/completeness contract. That
contract and its provider-local qualification are now complete; full Platform
application producer coverage remains a separate non-goal.

A deliberately billable uncovered-demand production exercise is the remaining
product-behavior closeout requirement. Focused unit and real-PostgreSQL tests
cover commit-before-paid-residual, exact-shape suppression, and paid
retirement, while production proved zero paid capacity at idle and through the
guarded request campaign under `max_live_paid_gpu_units: 0`. That zero cap also
forbids Spot overflow, so the final exercise must first activate an explicitly
approved positive ceiling, then prove only the uncovered residual starts as L4
Spot and that every paid claim and provider resource drains completely.

Historical recovery record: PR #1614 reconstructed the exact frozen committed-
intent profile for nine provider-present associations, and draft cleanup PR
#1615 removed its enumerated historical-digest verifier after the documented
horizon. Those rows were not the cause of the revision-470 incident; the cold-
controller retirement shelter was deployed and exercised. PR #1679 completed
the protocol-v7 purge/recreate rollout. The then-current lifecycle-94/gate-37
state and its protocol-v8 deployment, typed teardown, clean recreation, and
generation-37-to-38 activation were subsequently superseded by the later
completed lifecycle chronology. The historical `1.1.1448` deployment, gate-34
reauthorization, live duplicate-up/takeover, Kueue non-mutation, node health,
fresh telemetry, complete 180-second stale horizon, and primary 133/133
occupancy proof remain valid chronology, not current-state claims.
...
fix-forward through the SkyPilot Helm release. The current lifecycle-98 service
was created from the reviewed local `ml_models/providers/skypilot/**`
definition directly; no new or retained Platform PR is deployment authority.

`storage.enabled=false`; active control-plane Pods and lifecycle-98 fleet
workers have no EFS or PVC mount. The retired PVC, retained PV, and sole access
...
Capacity completion means Serve057 and the corrective retirement contract
remain deployed, the control-plane cohort is homogeneous and reauthorized,
`boltz-l4-fleet` continues to materialize 100% of fresh compatible capacity
that East scheduling and unchanged PHX Kueue admit, dashboard current-request
totals remain fresh, restart takeover succeeds without RWX/EFS, and every
required stale-horizon gate passes. Source-complete, deployed, activated, and
production-proven remain distinct states. Historical revision 594 / release
`1.1.1470` proved this dynamic boundary at 280/280 then 290/290 Ready. PR #1703
subsequently merged protocol-v10 cache scope and bounded controller memory; the
homogeneous `1.1.1477` / Helm-599 cohort and lifecycle-98 service renewed the
receipt at 314/314 Ready, including an exact 32-GPU replacement wave and its
+10/+30 horizons, then automatically advanced to 330/330 after the observer
published 16 more H200 slots and unchanged Kueue admitted their Workloads, with
zero paid claims or Spot replicas. Provider-local exact
async telemetry, actual-device completion evidence, and the guarded historical
10,000-request campaign are complete. The only open product-behavior exercise
is a genuinely uncovered bounded paid-Spot residual followed by complete
drain; the live cap-ten qualification envelope permits but does not create it.
...
one atomic transition, was initially deployed dark in revision 431, and was
later promoted. The exact
...
per-physical-pool executor; it merged in PR #1537, was initially deployed dark
in revision 418, and was later promoted. Second, every Pod was rejected from
`rescluster-k8s-prod-east1-preemptible-inference` because the launch omitted its
required Kueue queue label. At that historical point, the PHX contract was
LocalQueue `be`,
...
Historical completed sequence (non-executable; superseded by the current
closeout at the top of this document):
...
The current release inherits the following contracts from the historical A/B/C
pre-activation work. After the one-way generation gate activation, they remain
the retained compatibility and fail-closed boundaries; P2d and the exact
policy/config correction above define steady-state authority.
...
The same physical observation is the sole raw-capacity source for ordinary
demand placement and service-status free-capacity telemetry after
`SEQUENCED_ACTIVE`. Those readers require an integrity-valid exact-current
service claim before mapping its pool identities to the latest authoritative
physical-pool observation and carrying that observation's
admission/materialization high-waters into the post-observation debit. The
deployment inventory is poller bootstrap authority only: an observation made
from it is not demand- or status-visible until the broker claim compare-and-
swap commits and the reader re-reads that exact current claim. An absent,
stale, malformed, aliased, future-version, or otherwise mismatched
claim/observation is explicitly sequenced `UNKNOWN`; it receives zero
zero-cost allowance and never falls through to an ambient Kubernetes query or
legacy speculative probe. A current sequenced allocation is also required
before computing the paid residual. If it is unavailable, the controller
publishes the existing ordered zero paid-authority generation to revoke an
older positive plan, clears asynchronous logical actuation, and launches
nothing. The old context-keyed cache and its bounded speculative behavior run
only while the gate is exactly `LEGACY_ACTIVE` (or in the standalone SQLite
compatibility profile), and the worker rechecks that gate before provider I/O
and publication. This does not change N/N-1/N-2 retained-row decoding or
cleanup authority: cleanup continues to use each row's typed provider fence,
while fresh capacity has one observer and one PostgreSQL authority path.

A current service-controller restart obtains physical-pool identity from that
same exact-current claim set, never from a process-local previous-spec cache or
a fresh tokenless provider lookup. It accepts the claim only when its service
version, context/card/width/order, physical pool key, legacy key, and recomputed
worker-projection digest map exactly match the current immutable version
projection. A malformed or integrity-invalid current claim fails closed and is
neither heartbeated nor deleted.

A clean service or forward service-version update instead reads the reclaim
policy's typed provider-free pool inventory. The generic controller first
requires the plugin identity to equal the exact live gate identity, then
matches every configured context/card/width against the inventory and
recomputes every worker-projection digest from the current version's immutable
server-owned projection. A future-version claim, duplicate context, duplicate
physical card, unrecognized edge, old plugin without the typed method, or any
database/plugin/projection error clears local launch authority and preserves
durable evidence. The inventory itself grants no capacity. The poller first
runs the normal protocol-v2 observer, whose `V2_FENCED` physical-UID capture
must succeed; the broker then invokes the existing fresh provider-proof
`authorize_claim_set`, and the final PostgreSQL compare-and-swap rechecks the
gate, service owner, version, and projection before claim replacement. Only a
fresh committed observation can subsequently authorize allocation or launch.

The Boltz inventory method is a deterministic projection of the embedded
normalized fleet section. `fleet_bundle_sha256` hashes that entire section,
including every context, physical UID, accelerator, and width, so changing an
inventory atom rotates the live gate identity even when `policy_revision`
stays constant. The revision names accepted authorization semantics; merely
exposing an already-hashed identity projection does not change those
semantics. As a defense against an implementation bug that returns a different
typed projection under the same identity, claim authorization independently
re-derives and validates context, UID, card, width, and projected admission
from the embedded bundle before reading a provider receipt. The fenced
observer then proves the live UID, and the final PostgreSQL transaction
rechecks gate generation, service incarnation/controller owner, current
version, and immutable projection. Such a projection can therefore produce
only failed evidence and zero authority, never a grant.

The typed inventory deliberately allows one canonical access context per
deployment entry and rejects repeated context or physical-card atoms. The
current Boltz bundle already has that exact topology. A future deployment that
needs multiple authenticated aliases for one physical pool must extend this
typed contract and its bounded route validation explicitly; the controller
continues to fail `UNKNOWN` rather than infer aliases from ambient credentials
or historical service rows.
...
authority. Policy code that changes accepted evidence, authorization, or
enforcement must advance that revision; fleet and provider-contract changes
already rotate their domain-separated digests. A typed provider-free projection
of unchanged fields already covered by those digests and the activation proof
does not rotate the revision: it grants no effect authority, and older plugins
without the projection remain loadable but fail clean/new-version bootstrap
closed until the homogeneous matching image is present.
...
Kueue bundle before activation. This was an explicit first-activation gate, not
an operator bypass or a claim that the then-current east1 or Phoenix topology
was reclaimable; the current activation receipt is recorded above.
...
- The generalized non-pool association/profile/cohort and atomic binding IDs
  ...
  production-proven PR-#1678 cross-Pod takeover before supported teardown. At
  that checkpoint final acceptance still required v7 cleanup, fresh recreation,
  and a new scheduler-authorized refill/audit; those gates were later completed.
...
At that source-freeze snapshot, corrections 24 and 25 were present only in the
worktree and remained under focused test and adversarial review. They later
merged in PR #1703 and are deployed and production-proven by the lifecycle-98
receipt at the top of this document.
...
The deployed public server API version is 93 and the homogeneous writer cohort
uses worker projection protocol v10/cohort 10. Reserved-fill reconciliation
status keeps its independent minimum
...
protocol v9/cohort 9 remains only in the bounded compatibility/read window and
cannot start a fresh effect. Historical PostgreSQL API-request schema revision
010 was the activation floor and is likewise not a current-version claim. The
...
| 1e | Canonical birth for fresh lifecycle-fenced PostgreSQL non-pool services | Merged in PR #1621 at `bb81d16f1c2d194ec5bf488c1e1d87c8f44ee391` and exercised by historical lifecycles 84, 85, 91, 93, 94, 95, and 96/version 1 plus the current service incarnation. `add_service()` uses the existing Serve047-allowed adjacent update inside the service/version transaction, so PostgreSQL exposes only the final generic bound/route/demand/fill tuple. `_start()` verifies rather than promotes it. No schema, EFS, Helm, Terraform/Terragrunt, provider, pool, or SQLite change is included. Backfill, cross-Pod takeover, stale/quiescence, recreation, and revision-594 convergence proofs passed. |
...
| 2b | New immutable service version with task-owned Kubernetes overrides removed, `min_replicas: 0`, and exact non-null worker projections | Complete. Historical lifecycle 96/version 1 and current lifecycle 98/controller incarnation `ce80a485-8190-436b-bda6-a665aaf57fbe` preserve the canonical no-EFS projection through release `1.1.1477`. Helm storage is disabled and there is no SkyPilot PVC; PostgreSQL is authoritative. |
...
| 2b.2 | Protocol-v7/cohort-7 apt/SSH terminal-marker, interrupted-dpkg, and bounded rolling-cleanup correction | Complete in PR #1679 / `1.1.1449`; later releases retain its settlement contract. Its source-only and pending-rollout wording is historical. Historical lifecycle 96/gate 39 and revision-594 service superseded its old lifecycle-93/generation-35 rollout instructions; current release `1.1.1477` inherits the completed contract. No manual row/Pod deletion and no Kueue or infrastructure change was used. |
| 2c | P2c provider-independent route leases and safe zero-demand paid retirement (Serve051/API88) | PR #1531 was initially deployed dark and was later promoted. PR #1532's exact-owner fix is deployed as revision 410 / v1.1.1314. PR #1533's immutable route-contract fix is deployed as revision 411 / v1.1.1315 and removes the shared routing-lock dependency. Later fix-forwards removed the synchronous per-probe receipt-write bottleneck. Current-request telemetry remained fresh through cross-Pod takeover and the stale interval; artificial provider-stall behavior is source/test-qualified. Historical cleanup #1506 is closed/superseded and reserves no head. |
...
| 2g | Production full reserved backfill | **Complete for the current dynamic scheduler-authorized denominator.** Historical receipts reached 280/280, 290/290, and 383 Ready while exposing the v9 shared-cache overcount and old controller OOMs. PR #1703 removed those defects. Lifecycle 98 on the homogeneous `1.1.1477` / Helm-599 cohort reached and retained 314/314 Ready through the exact 32-GPU replacement wave and +10/+30 horizons. When the observer later published 16 more H200 slots and unchanged Kueue admitted their Workloads, it automatically advanced to 330/330 Ready: East 63 A100 plus 189 A100-80GB and PHX 78 H200. Every exact-card round and the public Infra snapshot report zero available capacity, every replica is reserved/non-Spot, and paid claims/waiters are zero. |
...
| 2n | Protocol-v10 shared-cache admission and bounded controller memory | **Merged, deployed, and production-proven.** PR #1703 merged at `fecb2b0f3` and is inherited by `1.1.1477` / Helm 599. V10 adds the digest-bound `replica_additive`/`node_shared_immutable` scope while preserving v8/v9 decode compatibility. Metadata-only single-flight presence reads, handle reuse, and one bounded reusable readiness executor replaced per-replica full-Pod materialization and per-tick pool churn. Lifecycle 98 renewed full occupancy and survived the exact 32-GPU wave plus stale/+10/+30 horizons. The active controller cgroup peaked at 5,101,756,416 bytes under its 8-GiB limit with zero `max`, OOM, or OOM-kill events; the production service child had 282 threads in the post-horizon observation under the structurally single max-256 readiness executor, and both controller Pods had zero restarts. Armando's separate test service had no lifecycle/version/spec mutation; its load balancers inherited the homogeneous Helm image. |
| 2o | Whole-service cancelled pre-effect retirement | **Source and real-PostgreSQL qualification complete; deployment and production proof pending.** Release `1.1.1494` / Helm 615 is the affected deployed cohort. The correction adds no schema, migration, activation, provider call, Kueue object, infrastructure resource, storage path, or alternate launch path. Supported purge of lifecycle 102 must atomically retire exact rows 49/50 while preserving association/request audit and `provider_evidence=NOT_QUERIED`; no manual deletion is allowed. |
...
Phase 2o qualification ran 12 production-shaped PostgreSQL cases covering both
terminal service states, repeated read-only observation, a two-row atomic
batch, a mixed no-effect/provider-absence batch, live-service closure, retained
Pod authority, and atomic rollback for terminal-request, quiescence,
cancellation, queue-pin, paid-claim, and provider-evidence mutations. All 12
passed. An expanded 35-case cleanup regression run passed 33; its two failures
are the same pre-existing stale mock-call expectations on unmodified
`v1.1.1494`, where already-canonical `ABSENT` evidence correctly avoids a
second provider probe. They are not caused by phase 2o and are not deployment
authority.
...
594 / release `1.1.1470`. PR #1703's protocol-v10 cache admission and bounded-
memory correction, PR #1706's convoy removal, and PR #1708's just-in-time fair
leasing are now deployed homogeneously as release `1.1.1477` / Helm 599.
Lifecycle 98 was created directly from the local provider source and renewed
full occupancy without a Platform PR, state migration, or teardown, recreation,
or service-spec change to Armando's test service. Its load balancers inherited
the homogeneous Helm image. The earlier direct-Helm purge/recreate,
...
qualification was still a rollout gate and was not inferred from this local
process evidence. The later Helm-572 cross-Pod takeover supplied that production
proof. Read-only production PostgreSQL inspection on 2026-08-20
...
are recorded at the top of this document. The scheduled +30 readback was
collected and its changed raw observation was adjudicated as a real eight-card
release that SkyPilot fully claimed and submitted. Those assignments later
settled and the zero-paid audit passed.
...
- The historical A/B/C no-deployment statement is superseded by the
  ...
  test-qualified; the current product closeout additionally requires one
  explicitly bounded live Spot residual and full drain. The +30 control-plane/
  HA/error horizon passed and its coincident eight-card release was fully
  claimed/submitted. Its then-open worker-readiness settlement and zero-paid
  audit were closed by the historical lifecycle-96/Helm-586 receipt and are
  further superseded by the revision-594 evidence at the top of this document.
...
Checked items below are retained receipts. Protocol-v10 cache admission,
controller-memory safety, clean recreation, the renewed denominator receipt,
and its final +30 horizon are complete. The current teardown correction and
supported purge must finish before the bounded authenticated L4 Spot
residual/drain exercise. No unchecked item authorizes a Kueue or non-SkyPilot
infrastructure change.

- [ ] Deploy phase 2o homogeneously with one immutable digest, retry the
  supported purge of lifecycle 102, and prove rows 49/50 retire atomically with
  exact association/request audit retained and `provider_evidence=NOT_QUERIED`.
  Require zero provider/Pod/cluster call, no manual deletion, and zero service,
  replica, admission, intent, claim, queue, pin, waiter, load-balancer, or live
  provider resource immediately, at +10, and at +30 minutes.
...
- [x] Merge and publish protocol v7 in PR #1679 / release `1.1.1449`, then
  direct-Helm deploy it homogeneously as revision 573 with `--reuse-values`.
- [x] Complete the supported lifecycle-93 purge, fresh lifecycle-94 recreation,
  and the historical generation-35 activation without manual row, Pod, or
  Workload deletion. Later normal fix-forwards advanced to lifecycle 96 and
  gate 39; none of those historical transitions may be repeated.
- [x] Prove 100% assignment of compatible East scheduler-fit capacity and PHX
  capacity admitted by Simone's unchanged Kueue policy. The historical
...
- [x] Merge and direct-Helm deploy one homogeneous API-93/projection-10/
  ...
  before fresh provider effects. Executors consume only the frozen scope. PR
  #1703 is merged and inherited by release `1.1.1477` / Helm 599; every current
  API/controller/executor Pod runs the exact same digest.
- [x] Prove controller memory remains bounded through normal service teardown,
  ...
  do not substitute a larger memory limit. Lifecycle 98 retained full occupancy
  through the exact 32-GPU refill and +10/+30 horizons. The active controller
  cgroup peaked at 5,101,756,416 bytes below its unchanged 8-GiB limit with zero
  memory-max/OOM events. The production service child had 282 threads in the
  post-horizon observation under the structurally single max-256 readiness
  executor; both controller Pods have zero restarts.
- [x] Preserve Armando's independently owned test service and use the already
  cleanly recreated production lifecycle 98/version 1 directly from tested
  local provider commit `a69fb3409ede26f98c02f7730f764021c1ef0146` for the
  protocol-v10 proof. It demonstrated v10
  ...
  telemetry. The exact 32-GPU qualification used only armed UID-preconditioned
  worker-Pod deletions; it did not migrate/relabel database rows, touch the test
  service, or use a Platform PR.
...
- [x] Open cross-linked draft projection cleanup PR #1619. Keep it draft until
  ...
  earlier v6 gates passed, and the v7 rollout plus later lifecycle-96
  convergence now supersede the old open refill settlement. Retarget the
  cleanup to remove only the v1-v4
...
- [x] Exercise the canonical fresh-service birth and move `boltz-l4-fleet` onto
  ...
  Lifecycle 93 and lifecycle 96 are historical. The live service is lifecycle
  98, controller incarnation `ce80a485-8190-436b-bda6-a665aaf57fbe`, version 1,
  on the homogeneous `1.1.1477` / Helm-599 cohort.
...
- [x] Apply #8652's production full-backfill service update from the clean
  ...
  source- and real-PostgreSQL-test-qualified, and the zero-cap request campaign
  proved reserved-covered suppression. The deliberately bounded live Spot
  residual/drain exercise remains open.
...
- [ ] Keep cross-linked draft cleanup PR #1633 blocked until the clean recreated
  ...
  historical production horizons, fresh recreation, refill, and zero-paid audit
  are complete. Only the cleanup-specific zero-scalar-`NULL` census remains
...
- [x] With zero authenticated demand, prove every fresh, authenticated,
  ...
  The revision-594 06:26 receipt closes this coherently at 280/280 Ready: East
  154 and PHX 126, with zero paid claims or Spot replicas. Five observed free
  PHX slots were not yet authorized at that instant; after confirmation the
  same service automatically reached a coherent 290/290 Ready by 06:44 without
  changing Kueue policy.
- [x] Prove the large-fill submission path is not an artificial serial prefix.
  PR #1686 bounds each physical pool independently. During the latest dynamic
  release, the published PHX grant progressed through 18, 25, 26, 34, 42, 52,
  54, 65, 82, 106, 124, 126, and then two further confirmations to 136, with
  each durable delta committed as a batch. The denominator remains policy-
  admissible capacity; SkyPilot must not manufacture Kueue admission to meet a
  nominal count.
- [x] Prove reserved supply is committed before the paid residual, no new Spot
  ...
  and throughout the guarded 10,000-request campaign under the zero cap. This
  closes reserved-covered suppression; a guarded genuinely uncovered paid-Spot
  launch/drain remains an open production exercise with an explicit cost
  boundary.
- [x] Adjudicate orphaned/ambiguous historical rows only from durable
  ...
  Lifecycles 89--91 retired through normal evidence-backed cleanup. PR #1687
  then atomically retired exact provider-free replica 465, its admission, and
  its intent while retaining historical association/request evidence;
  replacement 572 reached `POD_WAITING` without manual SQL.
- [x] Remove EFS from the SkyPilot runtime and correctness path. Helm storage is
  disabled and the live release has no SkyPilot PVC. Any unrelated deletion of
  a Terraform-managed shared filesystem is optional platform hygiene outside
  this convergence design, not a prerequisite or fallback.
- [x] Verify the active `DURABLE_FEED` dashboard exposes confirmed processing,
  ...
  stall injection is source/test-qualified rather than a production gate. The
  PostgreSQL idempotency ledger and explicit completeness contract now provide
  exact completed logical work for the protocol-covered provider cohort; the
  dashboard continues to label that coverage partial. Separately expose
  economic provenance (reserved fill, other
...
- [x] Complete the PostgreSQL-backed async request ledger, authenticated worker
  heartbeats/completions, and actual accelerator/device evidence. Revision
  594's provider-local qualification produced authenticated terminal evidence
  on A100, A100-80GB, and H200. HTTP admission remains distinct from terminal
  completion. The dashboard labels the exact PostgreSQL census protocol-
  covered/partial until all producers opt in; this receipt proves a fresh idle
  current-demand projection after the campaign, not a nonzero visual UI sample
  during it.
- [x] Run the guarded 10,000-logical-request campaign through the provider-local
  qualification harness. The bounded campaign respected authenticated reported capacity and
...
- [ ] After the complete capability, stale-writer, route, demand, and
  ...
  historical capability/horizon evidence and current revision-599 convergence
  are valid; only cleanup-specific zero-old-path predicates still apply.
- [ ] Keep atomic-authority cleanup #1556 stacked on feature PR #1555 until
  ...
  then merge it before declaring phase 2e complete. Capacity refill and the
  zero-paid census are complete; cleanup-specific partial-authority and
  stale-writer predicates remain the acceptance boundary.
